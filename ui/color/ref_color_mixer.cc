...
void AddBaselinePalette(ColorProvider* provider,
  ...
  // Zephyrus: the BASELINE ramp is where the brand lives.
  //
  // This is the whole trick, and it is Helium's (patches/helium/ui/
  // helium-color-scheme.patch, which swaps Google Blue for their 0x3551D2).
  // Every Material token downstream -- kColorSysPrimary, kColorSysBase, the
  // tab and toolbar colours, the WebUI variables -- is DERIVED from these
  // tones. Rewriting the ramp re-tints the entire browser coherently, in one
  // place, with the contrast relationships between surfaces intact.
  //
  // The alternative, which Zephyrus did until now, is to hold a hardcoded
  // Palette and paint each view from it. That gives an identity but no theme:
  // the browser cannot follow a colour the user picks, and pointing every
  // surface at one shared value to fix that just flattens the window (tried
  // 2026-09-10, reverted the same evening).
  //
  // This function is the DEFAULT palette only -- it runs when there is no
  // user_color. Pick a colour in Customize Chrome and AddRefColorMixer builds
  // the ramp from that seed instead, which is exactly the split we want:
  // Pomegranate is what Zephyrus looks like out of the box, and a theme is
  // still a real theme.
  //
  // Tones are the brand hue and chroma from #C6102E walked through the Material
  // tone stops in LCh, chroma reduced per stop until the result is inside sRGB
  // (see the generator in the session scratchpad). Tone 40 is pinned to the
  // exact brand value rather than the computed 0xBC102B -- the identity colour
  // should be the identity colour.
  mixer[kColorRefPrimary0] = {SkColorSetRGB(0x00, 0x00, 0x00)};
  mixer[kColorRefPrimary10] = {SkColorSetRGB(0x40, 0x02, 0x01)};
  mixer[kColorRefPrimary20] = {SkColorSetRGB(0x68, 0x01, 0x12)};
  mixer[kColorRefPrimary25] = {SkColorSetRGB(0x7C, 0x02, 0x18)};
  mixer[kColorRefPrimary30] = {SkColorSetRGB(0x92, 0x01, 0x1E)};
  mixer[kColorRefPrimary40] = {SkColorSetRGB(0xC6, 0x10, 0x2E)};
  mixer[kColorRefPrimary50] = {SkColorSetRGB(0xDF, 0x33, 0x40)};
  mixer[kColorRefPrimary60] = {SkColorSetRGB(0xFC, 0x57, 0x59)};
  mixer[kColorRefPrimary70] = {SkColorSetRGB(0xFE, 0x89, 0x83)};
  mixer[kColorRefPrimary80] = {SkColorSetRGB(0xFF, 0xB3, 0xAD)};
  mixer[kColorRefPrimary90] = {SkColorSetRGB(0xFF, 0xDA, 0xD6)};
  mixer[kColorRefPrimary95] = {SkColorSetRGB(0xFF, 0xED, 0xEB)};
  mixer[kColorRefPrimary99] = {SkColorSetRGB(0xFF, 0xFB, 0xFB)};
  mixer[kColorRefPrimary100] = {SkColorSetRGB(0xFF, 0xFF, 0xFF)};

  // Secondary is deliberately ALMOST grey -- the same hue at 12% of the
  // chroma.
  //
  // Nothing OS is a greyscale ramp and ONE accent, so a second saturated hue
  // would be a second accent and would spend exactly the value the red has.
  // Helium made the same move in the other direction (their secondary went
  // from teal-blue to a neutral slate). A trace of the brand hue is enough to
  // stop the greys reading as dead, which matters because kColorSysOmniboxContainer
  // resolves to kColorRefSecondary15 in dark mode -- the omnibox is the largest
  // thing this ramp paints.
  mixer[kColorRefSecondary0] = {SkColorSetRGB(0x00, 0x00, 0x00)};
  mixer[kColorRefSecondary10] = {SkColorSetRGB(0x27, 0x17, 0x16)};
  mixer[kColorRefSecondary12] = {SkColorSetRGB(0x2C, 0x1B, 0x1A)};
  mixer[kColorRefSecondary15] = {SkColorSetRGB(0x32, 0x22, 0x20)};
  mixer[kColorRefSecondary20] = {SkColorSetRGB(0x3E, 0x2C, 0x2A)};
  mixer[kColorRefSecondary25] = {SkColorSetRGB(0x49, 0x37, 0x35)};
  mixer[kColorRefSecondary30] = {SkColorSetRGB(0x55, 0x42, 0x40)};
  mixer[kColorRefSecondary35] = {SkColorSetRGB(0x61, 0x4E, 0x4C)};
  mixer[kColorRefSecondary40] = {SkColorSetRGB(0x6E, 0x5A, 0x58)};
  mixer[kColorRefSecondary50] = {SkColorSetRGB(0x87, 0x72, 0x70)};
  mixer[kColorRefSecondary60] = {SkColorSetRGB(0xA2, 0x8C, 0x89)};
  mixer[kColorRefSecondary70] = {SkColorSetRGB(0xBD, 0xA6, 0xA4)};
  mixer[kColorRefSecondary80] = {SkColorSetRGB(0xD9, 0xC1, 0xBF)};
  mixer[kColorRefSecondary90] = {SkColorSetRGB(0xF5, 0xDD, 0xDB)};
  mixer[kColorRefSecondary95] = {SkColorSetRGB(0xFF, 0xED, 0xEB)};
  mixer[kColorRefSecondary99] = {SkColorSetRGB(0xFF, 0xFB, 0xFB)};
  ...
  if (color_mode == ColorProviderKey::ColorMode::kDark &&
    ...
    mixer[kColorRefNeutralVariant10] = {SkColorSetRGB(0x18, 0x1C, 0x22)};
    mixer[kColorRefNeutralVariant15] = {SkColorSetRGB(0x23, 0x26, 0x2C)};
    mixer[kColorRefNeutralVariant20] = {SkColorSetRGB(0x2D, 0x30, 0x38)};
    mixer[kColorRefNeutralVariant30] = {SkColorSetRGB(0x44, 0x47, 0x4E)};
    mixer[kColorRefNeutralVariant40] = {SkColorSetRGB(0x5B, 0x5E, 0x66)};
    mixer[kColorRefNeutralVariant50] = {SkColorSetRGB(0x74, 0x77, 0x7F)};
    mixer[kColorRefNeutralVariant60] = {SkColorSetRGB(0x8E, 0x90, 0x99)};
    mixer[kColorRefNeutralVariant70] = {SkColorSetRGB(0xA9, 0xAB, 0xB4)};
    mixer[kColorRefNeutralVariant80] = {SkColorSetRGB(0xC4, 0xC6, 0xD0)};
    mixer[kColorRefNeutralVariant90] = {SkColorSetRGB(0xEB, 0xED, 0xF7)};
    mixer[kColorRefNeutralVariant95] = {SkColorSetRGB(0xF9, 0xF9, 0xFF)};
    mixer[kColorRefNeutralVariant99] = {SkColorSetRGB(0xFD, 0xFB, 0xFF)};
