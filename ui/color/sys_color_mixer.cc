...
void AddThemedSysColorOverrides(ColorMixer& mixer,
  ...
  // Experimentation.
  // Zephyrus (from Helium): one step further off the base, so the omnibox pill
  // stays a distinct plane from the toolbar it sits in under a themed palette.
...
void AddGrayscaleSysColorOverrides(ColorMixer& mixer,
  ...
  // Zephyrus (from Helium): a toned divider. The default variant is close
  // enough to the surface it divides that hairlines disappear on the grey
  // theme, which is the theme Zephyrus ships on.
