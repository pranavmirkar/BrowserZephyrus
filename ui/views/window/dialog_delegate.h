...
class VIEWS_EXPORT DialogDelegate : public WidgetDelegate {
 public:
  struct Params {
    ...
    // Zephyrus: keep the Views custom (frameless) frame but make the WIDGET
    // OPAQUE instead of translucent. A GPU-composited translucent window renders
    // its transparent regions black when DirectComposition is disabled (our AMD
    // scroll-stutter workaround), and dialogs hosting a WebView cannot fall back
    // to software compositing (it blanks the web contents). An opaque window
    // sidesteps both; rounded corners + shadow then come from DWM. Only honored
    // for custom_frame dialogs.
    bool opaque_custom_frame = false;
  ...
  // Zephyrus: see Params::opaque_custom_frame.
  void set_opaque_custom_frame(bool opaque) {
    params_.opaque_custom_frame = opaque;
  }
  bool opaque_custom_frame() const { return params_.opaque_custom_frame; }
