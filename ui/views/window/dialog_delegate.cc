...
Widget::InitParams DialogDelegate::GetDialogWidgetInitParams(
  ...
  if (!dialog || dialog->use_custom_frame()) {
    // Zephyrus: an opaque custom frame keeps the frameless Views look but avoids
    // the translucent-window black border on a DComp-disabled AMD build (see
    // Params::opaque_custom_frame). It uses the OS drop shadow instead of the
    // bubble frame's compositor shadow, so keep native shadowing in that case.
    const bool opaque = dialog && dialog->opaque_custom_frame();
    params.opacity = opaque ? Widget::InitParams::WindowOpacity::kOpaque
...
std::unique_ptr<FrameView> DialogDelegate::CreateDialogFrameView(
  ...
  // Zephyrus: an opaque custom-frame dialog (see Params::opaque_custom_frame)
  // must NOT draw the compositor drop shadow. The shadow is a semi-transparent
  // gradient meant to blend with the desktop through a translucent window; on an
  // opaque window it instead paints over the window's own (black) background and
  // reads as a hard dark border. Drop it — DWM supplies the OS window shadow.
  const BubbleBorder::Shadow kShadow =
      (delegate && delegate->opaque_custom_frame()) ? BubbleBorder::NO_SHADOW
