...
void DesktopWindowTreeHostWin::Init(const Widget::InitParams& params) {
  ...
  // Zephyrus: when DirectComposition is disabled (our AMD scroll-stutter
  // workaround sets --disable-direct-composition), a GPU-composited translucent
  // window cannot deliver per-pixel alpha to DWM, so its transparent regions
  // (rounded-corner cutouts and drop shadow) render as opaque black. Force
  // software compositing for the translucent popups (bubbles, dropdowns, menus)
  // so they take the UpdateLayeredWindow path, which composites alpha correctly
  // without DComp. EXCLUDES TYPE_WINDOW: that covers the main browser frame AND
  // the browser-modal dialogs (e.g. the Settings popup), which host a WebView —
  // a software-composited window cannot display GPU-composited web contents, so
  // forcing software there blanks the page. Those windows keep their GPU
  // compositor; their black border is addressed by making them opaque instead.
  // Gated on the switch, so Intel/NVIDIA (DComp on) keep GPU-composited popups.
  bool force_software_compositing = params.force_software_compositing;
  if (!force_software_compositing && message_handler_->is_translucent() &&
      params.type != Widget::InitParams::TYPE_WINDOW &&
      base::CommandLine::ForCurrentProcess()->HasSwitch(
          "disable-direct-composition")) {
    force_software_compositing = true;
  }

  CreateCompositor(force_software_compositing);
