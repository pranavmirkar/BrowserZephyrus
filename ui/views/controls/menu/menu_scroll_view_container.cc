...
#if BUILDFLAG(IS_WIN)
  // Zephyrus: DWM rounds and shadows the menu window (see ApplyWindowShaping
  // in menu_host.cc). A BubbleBorder shadow on top of that would double up,
  // and its reserved transparent margin would push DWM's own rounding out to a
  // rectangle noticeably larger than the menu.
