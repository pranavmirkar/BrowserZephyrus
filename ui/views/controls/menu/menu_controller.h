...
class VIEWS_EXPORT MenuController final : public gfx::AnimationDelegate,
 ...
 public:
  ...
  // ZEPHYRUS: the bounds the menu was anchored to, in SCREEN coordinates.
  // Needed to aim the app menu's nub at the button that opened it; the menu
  // itself otherwise has no idea where its anchor is.
  const gfx::Rect& GetZephyrusAnchorBounds() const {
    return state_.initial_bounds;
  }
