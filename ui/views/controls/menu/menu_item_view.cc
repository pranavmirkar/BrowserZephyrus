...
void MenuItemView::PaintMinorIconAndText(gfx::Canvas* canvas, SkColor color) {
  ...
  if (!minor_text.empty()) {
    ...
    // Zephyrus: accelerators sit one step below the label (13px label / 12px
    // accelerator in the design), so the shortcut reads as secondary rather
    // than competing with the command name. Upstream paints both at the label
    // size; there is no MenuConfig field for a minor-text font, so derive it.
    render_text->SetFontList(GetFontList().DeriveWithSizeDelta(-1));
...
MenuItemView::MenuItemDimensions MenuItemView::CalculateDimensions() const {
  ...
  // Determine the length of the right-side text. Measured with the same
  // one-step-smaller font PaintMinorIconAndText draws it in, so the reserved
  // width matches what is actually painted.
  std::u16string minor_text = GetMinorText();
  dimensions.minor_text_width =
      (minor_text.empty()
           ? 0
           : gfx::GetStringWidth(minor_text,
                                 font_list.DeriveWithSizeDelta(-1)));
