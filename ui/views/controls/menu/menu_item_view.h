...
class VIEWS_EXPORT MenuItemView : public View, public LayoutDelegate {
 ...
 public:
  ...
  // ZEPHYRUS: set on a ROOT menu item to give its menu a nub aimed at the
  // control that opened it. Off by default: a context menu opens at the cursor
  // and has nothing to point at, so only genuinely anchored menus set this.
  void SetZephyrusWantsNub(bool wants) { zephyrus_wants_nub_ = wants; }
  bool zephyrus_wants_nub() const { return zephyrus_wants_nub_; }
 ...
 private:
  ...
  // ZEPHYRUS: see SetZephyrusWantsNub().
  bool zephyrus_wants_nub_ = false;
