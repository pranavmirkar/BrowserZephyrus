...
// Zephyrus: let DWM shape and shadow the menu window.
//
// This is what survived a long, failed attempt to give menus a blurred
// backdrop. DWM accepts DWMWA_SYSTEMBACKDROP_TYPE on these windows and reports
// it back, but never SAMPLES what is behind them — it paints a flat fallback
// colour instead, identical over light and dark content. That held with the
// window layered and not, GPU- and software-composited, with and without
// WS_EX_NOREDIRECTIONBITMAP, on an activating window, and with the older
// ACCENT_ENABLE_BLURBEHIND. Best understanding: the system backdrop does not
// composite behind Chromium's DirectComposition-presented content. Do not
...
void MenuHost::InitMenuHost(const InitParams& init_params) {
  ...
  //
  // (Zephyrus briefly set this false to escape the WS_EX_LAYERED path, which
  // blocks a DWM blurred backdrop. Restored: the backdrop never rendered even
  // once every precondition was satisfied — see ApplyWindowShaping above.)
  params.force_software_compositing = true;
#endif
  [[maybe_unused]] const bool translucent =
      params.opacity == Widget::InitParams::WindowOpacity::kTranslucent;
  Init(std::move(params));

#if BUILDFLAG(IS_WIN)
  if (translucent) {
    ApplyWindowShaping(HWNDForWidget(this));
  }
#endif
