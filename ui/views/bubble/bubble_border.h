...
class VIEWS_EXPORT BubbleBorder : public Border {
 public:
  ...
  // ZEPHYRUS: 10, was 8. Matches the design's nub and, more importantly, this
  // is what the layout RESERVES above the bubble -- draw a taller nub than this
  // and the extra is clipped off.
  static constexpr int kVisibleArrowLength = 10;

  // Radius (half-width) of the visible arrow, when one is present.
  // ZEPHYRUS: half-width. 10 gives the design's 20px-wide nub.
  static constexpr int kVisibleArrowRadius = 10;
  ...
  // ZEPHYRUS: the full outline including the nub, or nullopt when this bubble
  // has no nub on its top edge.
  //
  // Shared by the BORDER (which draws the shadow) and the BACKGROUND (which
  // draws the fill) so the two cannot disagree. They did: the shadow was drawn
  // from the nub path while the fill was still a plain rounded rect, so the nub
  // had no fill and showed the shadow colour through it.
  //
  // Takes the view because `visible_arrow_rect_` is in SCREEN coordinates and
  // has to be converted into the view's space first. Feeding the raw screen x
  // into a view-local path puts the nub off by the window's position on the
  // desktop, which looks like a nub that ignores its anchor.
  std::optional<SkPath> GetZephyrusNubOutline(const View& view,
                                              const SkRRect& r_rect) const;

  // ZEPHYRUS: aim a nub at this SCREEN x, for surfaces that know their anchor
  // but do not go through the bubble positioning code.
  //
  // Bubbles get the nub from `visible_arrow_rect_`, which BubbleBorder fills in
  // while computing their bounds. A MENU is positioned by MenuController and
  // never calls that, so its arrow rect is always empty. This is the way in for
  // those: the caller supplies the anchor, and the x is converted to view space
  // at paint time, when the view is actually in a widget.
  void set_zephyrus_nub_screen_x(std::optional<float> x) {
    zephyrus_nub_screen_x_ = x;
  }
  bool has_zephyrus_nub() const { return zephyrus_nub_screen_x_.has_value(); }
 ...
 private:
  ...
  // ZEPHYRUS: see set_zephyrus_nub_screen_x().
  std::optional<float> zephyrus_nub_screen_x_;
