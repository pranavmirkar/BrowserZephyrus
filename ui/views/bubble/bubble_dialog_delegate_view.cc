...
class BubbleDialogDelegate::AnchorWidgetObserver : public WidgetObserver,
 ...
 public:
  ...
  void OnWidgetThemeChanged(Widget* widget) override {
    ...
    //
    // Zephyrus: NULL-CHECKED. The bubble's own widget is gone from the moment
    // WidgetDelegate::WidgetDestroying() clears it, while THIS observer lives
    // on until the delegate itself is deleted -- which, for a bubble closed
    // with Widget::Close(), is a posted task later. A theme change landing in
    // that gap crashed the browser on a null `this`, and changing the colour
    // from the customize panel is exactly when one lands: ThemeService ->
    // BrowserWidget::UserChangedTheme -> the frame's theme propagation.
    if (Widget* bubble_widget = owner_->GetWidget()) {
      bubble_widget->ThemeChanged();
    }
  }
