...
// Zephyrus: the Material 3 corner radius scale, all ten steps.
//
// M3 defines shape as a fixed size-based scale and then assigns each component
// a step from it. A component's radius is therefore never a judgement call --
// it is looked up. See chrome/browser/zephyrus/M3_UI_OVERHAUL.md, Rule 1.
//
// These replace an ad-hoc set (5 / 10 / 16 / 20 / 26) that sat on no scale at
// all. Names follow M3's own, so a value can be checked against the spec
// without a translation table.
//
// Nested shapes do NOT pick from this scale -- their radius is derived by
// zephyrus::m3::ConcentricInner(outer, padding). Rule 2.
enum class ShapeSysTokens {
  ...
  kLargeIncreased,       // 20
  kExtraLarge,           // 28
  kExtraLargeIncreased,  // 32
  kExtraExtraLarge,      // 48
  kFull,                 // pill: half the shorter side
};
