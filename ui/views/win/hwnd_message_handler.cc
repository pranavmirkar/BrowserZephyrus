...
void HWNDMessageHandler::FrameTypeChanged() {
  ...
  // Zephyrus: a custom-drawn frame has no DWM transition to perform.
  //
  // PerformDwmTransition() opens with CHECK(IsFrameSystemDrawn()), and
  // `dwm_transition_desired_` is only ever set true just above -- under that
  // same condition. So with a custom-drawn frame the test below reads
  // `!false || ...` and walks straight into the CHECK.
  //
  // Upstream never reaches it because ThemeHelperWin::ShouldUseNativeFrame()
  // is only false while a custom theme is installed. Zephyrus returns false
  // unconditionally (we always draw our own title bar), so the frame is ALWAYS
  // custom-drawn and every browser theme change came through here and aborted
  // the browser -- which is why picking a colour in Customize Chrome killed the
  // window. Confirmed from the minidump: exception 0x80000003 with
  // PerformDwmTransition/FrameTypeChanged on the faulting thread.
  //
  // The regeneration itself still has to happen -- it is the entire point of
  // BrowserWidget::RegenerateFrameOnThemeChange() -- and it is not DWM
  // specific: PerformDwmTransition()'s body is exactly these two calls plus
  // clearing a flag that is already false here.
