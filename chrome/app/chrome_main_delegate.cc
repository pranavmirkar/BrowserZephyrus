...
#if BUILDFLAG(IS_WIN)
  // Zephyrus: DirectComposition is ON, as upstream intends. This used to probe
  // for an AMD adapter (PCI vendor 0x1002) and append
  // --disable-direct-composition, on the theory that the DComp present path
  // stuttered on the integrated Radeon. That theory was retired on 2026-08-10
  // after it was measured properly:
  //
  //   * The original evidence compared our build against stock Chrome M150 and
  //     blamed the milestone-shaped gap on DComp. Chrome is now 151.0.7922 —
  //     the same milestone as our 151.0.7913 — and runs DComp smoothly on the
  //     very same Radeon 680M. There is no version gap left to hide behind.
  //   * A/B on a Release binary: DComp ON measured BETTER than OFF on our own
  //     build (5 hitches vs 9, p95 present gap 17.8ms vs 27.3ms).
  //   * Maximized, on a WARM profile, ours hitches 3 times against Chrome's 3.
  //     The 20-hitch "maximized is terrible" reading came from tracing on a
  //     cold profile, where first-run filter-list work drowned the signal.
  //   * Our build presents ~20 DComp overlays per commit against Chrome's 1,
  //     but forcing that to 1 (--disable-features=DelegatedCompositing) changed
  //     nothing, so it is a curiosity, not the cause.
  //
  // The residual intermittent lag is CPU contention — ~3300 URL-loader body
  // reads costing 653ms of background-thread time per run against Chrome's
  // 93ms — which starves the GPU vsync thread so its Sleep(vsync_interval)
  // overruns to 101ms (Chrome: 6ms). That is the resume-from-idle hitch, and
  // it is a networking problem, not a compositing one. Chase it there.
  //
  // Turning DComp back on also retires the whole workaround chain it forced:
  // translucent popups no longer need software compositing (the gate in
  // desktop_window_tree_host_win.cc keys off the switch and now stays shut),
  // and blurred/glass surfaces become possible at all — a layered window can
  // never have a DWM backdrop (see menu_host.cc).
  //
  // --zephyrus-disable-dcomp restores the old behaviour for A/B testing.
  if (is_browser && command_line.HasSwitch("zephyrus-disable-dcomp")) {
    base::CommandLine::ForCurrentProcess()->AppendSwitch(
        "disable-direct-composition");
  }

  // THE ACTUAL CAUSE of the "Zephyrus scrolls and types worse than Chrome on
  // the same machine" report, found 2026-08-10.
  //
  // testing/variations/fieldtrial_testing_config.json turns on
  // UseCompositorClockVSyncInterval for Windows. That config is compiled in and
  // ACTIVE for unbranded builds — we set is_chrome_branded=false — while stock
  // Chrome uses real Finch, where this experiment is not on. So our build alone
  // opts into VSyncThreadWinDComp, whose WaitForVSyncImpl falls back to a FLAT
  // Sleep(kMinimumFallbackTimeout = 100ms) whenever
  // DCompositionWaitForCompositorClock returns anything but WAIT_OBJECT_0 —
  // which it does here. Input arriving inside that window waits up to 100ms:
  // exactly the felt "typing lands late, scroll hitches on resume".
  //
  // Measured (maximized, warm profile, same page): ours 31 sleeps averaging
  // ~100ms each and a 216ms max task on the vsync thread, against Chrome's 16
  // sleeps of 6ms and a 15.5ms max. The sleeps are a hard constant, not a
  // scheduling smear — a histogram of them lands entirely in the >=99ms bucket.
  //
  // Disabling DComp used to hide this by routing to VSyncThreadWinDXGI, which
  // sleeps the REAL vsync interval. That is why the old AMD workaround appeared
  // to work, and why it cost us every translucent-popup capability for nothing.
  //
  // Targeted rather than --disable-field-trial-config, so we change one
  // experiment instead of silently flipping every other one at the same time.
  // The strategic fix is disable_fieldtrial_testing_config=true in args.gn, so
  // a product build stops shipping in-progress experiments at all; that is a
  // full rebuild and a separate decision.
  if (is_browser) {
    auto* const current = base::CommandLine::ForCurrentProcess();
    std::string disabled =
        current->GetSwitchValueASCII(::switches::kDisableFeatures);
    if (!disabled.empty()) {
      disabled += ",";
    }
    disabled += "UseCompositorClockVSyncInterval";
    current->AppendSwitchASCII(::switches::kDisableFeatures, disabled);
  }
#endif  // BUILDFLAG(IS_WIN)
