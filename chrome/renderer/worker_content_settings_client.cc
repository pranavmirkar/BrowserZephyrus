...
WorkerContentSettingsClient::WorkerContentSettingsClient(
  ...
  // Zephyrus: take the creating frame's fingerprint seed, so a worker
  // perturbs a canvas exactly as its own document would. Copied here rather
  // than derived independently — the seed is per (origin, session), and a
  // worker that disagreed with its document about the same canvas would
  // reproduce the cross-read inconsistency that got four of the top 200 sites
  // serving bot challenges.
  if (auto* seed_agent =
          zephyrus_privacy::FingerprintSeedAgent::Get(render_frame)) {
    zephyrus_seed_ = seed_agent->Seed();
    zephyrus_surface_mask_ = seed_agent->SurfaceMask();
  }
...
WorkerContentSettingsClient::WorkerContentSettingsClient(
    ...
    : is_unique_origin_(other.is_unique_origin_),
      ...
      // Declared after frame_token_ in the header, so initialised after it
      // here too — the initialiser list must follow declaration order.
      zephyrus_seed_(other.zephyrus_seed_),
      zephyrus_surface_mask_(other.zephyrus_surface_mask_) {
...
WorkerContentSettingsClient::GetZephyrusFingerprintSeed() {
  return zephyrus_seed_;
}

uint32_t WorkerContentSettingsClient::GetZephyrusFingerprintSurfaceMask() {
  return zephyrus_surface_mask_;
}
