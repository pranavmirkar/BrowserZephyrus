...
bool ThemeHelperWin::ShouldUseNativeFrame(
  ...
  // Zephyrus: use the custom-drawn OpaqueBrowserFrameView (not the native
  // BrowserFrameViewWin) so the toolbar can serve as the title bar with the
  // centered URL bar and in-toolbar window controls.
  return false;
}
