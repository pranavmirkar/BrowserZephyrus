...
void SessionServiceBase::BuildCommandsForTab(
  ...
  // Zephyrus Private Workspace: never write an off-the-record tab to the
  // session file. ShouldTrackBrowser() decides per WINDOW, so once a window can
  // host tabs from two profiles (PW-6) a regular window would carry its private
  // tabs' URLs to disk and restore them after a restart. Filtering here is
  // per-tab and fail-safe: it can only ever omit a tab, never add one.
  if (tab->GetBrowserContext() && tab->GetBrowserContext()->IsOffTheRecord()) {
    return;
  }
