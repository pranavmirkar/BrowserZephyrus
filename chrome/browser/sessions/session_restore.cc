 ...
 private:
  ...
  Browser* FinishedTabCreation(bool succeeded,
    ...
    if (!created_tabbed_browser && always_create_tabbed_browser_) {
      ...
      // Zephyrus: upstream opened the new tab page here when a restore
      // produced no tabbed browser and nothing was supplied on the command
      // line. We have no new-tab page, so the window simply opens with no tabs
      // and shows the empty backdrop; the user starts with Ctrl+T. This is the
      // path a normal launch actually takes (SYNCHRONOUS_RESTORE), so it — not
      // the startup-tab providers — is what governs what a fresh window shows.
  ...
  void RestoreTabsToBrowser(
    ...
    // Zephyrus: the window is normally shown as a side effect of restoring its
    // selected tab, so a session whose tabs were ALL dropped (we skip restoring
    // new-tab pages) would leave the browser alive but never shown — the app
    // launches into nothing. Show it directly here: ShowBrowser() can't be used
    // because it activates a tab and DCHECKs that one exists. The window comes
    // up on the empty backdrop.
    if (browser != browser_ && browser->tab_strip_model()->empty()) {
  ...
  void RestoreTab(const sessions::SessionTab& tab,
    ...
    // Zephyrus: never restore a new-tab page. Sessions saved before the NTP was
    // removed still carry these entries, and restoring one puts the very page
    // we abolished back on screen. Dropping it is not lossy: it holds no user
    // content, and a window left with no tabs shows the empty backdrop.
    {
      const GURL& restored_url =
          tab.navigations[GetNavigationIndexToSelect(tab)].virtual_url();
      if (restored_url == chrome::ChromeUINewTabURLAsGURL() ||
          restored_url == GURL(chrome::kChromeUINewTabPageURL)) {
        return;
      }
    }
