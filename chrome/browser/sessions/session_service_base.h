...
class SessionServiceBase : public sessions::CommandStorageManagerDelegate,
 ...
 public:
  ...
  // ZEPHYRUS: does this TAB belong to the profile this service writes for?
  //
  // Chromium decides session tracking per WINDOW -- ShouldTrackBrowser()
  // compares browser->GetProfile() to profile() and everything below it is
  // gated on ShouldTrackChangesToWindow(window_id). That is sound while a
  // window's tabs all share its profile, and it is a PRIVACY LEAK the moment
  // they do not: a window tracked as "regular" writes every tab it hosts to the
  // on-disk session file, including tabs belonging to another profile, and they
  // come back after a restart.
  //
  // Zephyrus workspaces are per-profile and swap inside one window, so that
  // situation is now normal rather than exotic. These two members make the gate
  // per tab: a tab whose profile does not match is recorded once at insertion
  // and every later command referencing its id is dropped.
  //
  // The id set is the load-bearing half. Most tab-level entry points receive
  // only (window_id, tab_id) and have no WebContents to ask, so the profile
  // check has to happen where the contents ARE available and be remembered.
  bool ShouldTrackTab(SessionID tab_id) const;
 ...
 protected:
  ...
  // Tabs seen at insertion whose profile did not match ours. See
  // ShouldTrackTab().
  std::set<SessionID> untracked_tabs_;
