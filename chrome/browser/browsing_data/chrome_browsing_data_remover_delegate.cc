...
void ChromeBrowsingDataRemoverDelegate::RemoveEmbedderData(
  ...
  if ((remove_mask & constants::DATA_TYPE_HISTORY) && may_delete_history) {
    ...
    // Zephyrus Privacy Intelligence keeps its own per-site tracker history. It
    // is browsing history by any honest reading, so clearing history must clear
    // it too — leaving it behind would mean the privacy feature is the one
    // thing that remembers where you went (§5.2.2).
    if (auto* zephyrus_privacy =
            zephyrus_privacy::PrivacyIntelligenceServiceFactory::
                GetForBrowserContext(profile_)) {
      if (delete_begin_.is_null() && delete_end_.is_max()) {
        // "Everything" takes the lifetime counters with it. §5.2.1 exempts them
        // from RETENTION, not from an explicit clear.
        zephyrus_privacy->DeleteAllBrowsingData();
      } else {
        // A ranged clear spares the lifetime totals: they are a single row of
        // three numbers with no time dimension, so there is no honest way to
        // subtract a window from them, and zeroing them would delete data the
        // user did not ask to delete.
        zephyrus_privacy->DeleteRange(delete_begin_, delete_end_);
      }
    }
