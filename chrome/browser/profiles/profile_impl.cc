...
void ProfileImpl::OnLocaleReady(CreateMode create_mode) {
  ...
  // Zephyrus "Auto reset browser IDs", per-profile half. The toggle lives in
  // local state because the feature is browser-wide, not per-profile.
  zephyrus_privacy::MaybeResetProfileIdentifiers(
      GetPrefs(),
