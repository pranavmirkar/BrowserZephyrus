...
void RegisterProfilePrefs(user_prefs::PrefRegistrySyncable* registry,
  ...
  // lock that quietly turns itself off is worse than no lock.
  registry->RegisterBooleanPref("zephyrus.private_workspace.lock_enabled",
                                false);
  ...
  // Read-only stats surfaced to the settings page (lifetime blocked count and
  // active rule count).
  registry->RegisterIntegerPref("zephyrus.adblock.total_blocked", 0);
  registry->RegisterIntegerPref("zephyrus.adblock.rule_count", 0);
