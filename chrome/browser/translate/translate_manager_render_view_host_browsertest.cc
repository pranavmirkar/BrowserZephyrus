...
class TranslateManagerRenderViewHostNoOverrideTest
 ...
 public:
  // Zephyrus: the supported-language roster fetch is off by default, because it
  // contacts Google without the user asking to translate anything (see
  // kTranslateLanguageListFetch in translate_features.cc). The machinery is
  // gated rather than removed, so this fixture opts back in -- otherwise no
  // request is issued and SimulateSupportedLanguagesURLFetch has nothing to
  // complete, leaving the roster at its 132 compiled-in defaults.
  //
  // This has to be a ScopedFeatureList in the constructor, NOT an
  // --enable-features switch in SetUpCommandLine: the parent constructor
  // already installs its own ScopedFeatureList, and that runs before
  // SetUpCommandLine, so a switch appended there is overridden and silently
  // does nothing. Layering a second ScopedFeatureList here runs after the
  // parent's and composes with it.
  TranslateManagerRenderViewHostNoOverrideTest() {
    language_list_feature_.InitAndEnableFeature(kTranslateLanguageListFetch);
  }

  void SetUpCommandLine(base::CommandLine* command_line) override {}

 private:
  base::test::ScopedFeatureList language_list_feature_;
};
