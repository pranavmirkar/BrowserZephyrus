...
GCMProfileServiceFactory::GCMProfileServiceFactory()
  ...
  // Zephyrus: the lazy-check-in gate reads notification content settings.
...
GCMProfileServiceFactory::BuildServiceInstanceForBrowserContext(
  ...
  // Zephyrus: lazy GCM check-in. GCM (web push) only registers this device
  // with Google once at least one site has actually been granted notification
  // permission; a wildcard/policy default of ALLOW also counts. Until then a
  // fresh profile never contacts android.clients.google.com. Evaluated live
  // on every GCM start attempt.
  if (service->driver()) {
    service->driver()->SetZephyrusStartAllowedCallback(base::BindRepeating(
