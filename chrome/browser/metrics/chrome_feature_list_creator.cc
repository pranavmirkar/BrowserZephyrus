...
void ChromeFeatureListCreator::CreateFeatureList() {
  ...
  // Zephyrus "Auto reset browser IDs": discard the installation-scoped
  // identifiers here, in the gap between local state existing and anything
  // reading it.
  //
  // This placement is the whole feature. CreateMetricsServices() below builds
  // the MetricsStateManager, which reads the client ID and the low-entropy
  // source and GENERATES them if absent, and SetUpFieldTrials() then randomises
  // this session from that entropy. Clearing after either point would leave the
  // old values live in memory for the session, and any later write would put
  // them straight back on disk -- the rotation would stop silently, which is
  // the worst way for a privacy feature to fail.
  zephyrus_privacy::MaybeResetLocalStateIdentifiers(local_state());
