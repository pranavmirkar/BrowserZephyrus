...
void ChromeContentBrowserClient::RegisterProfilePrefs(
  ...
  // Zephyrus: <a ping> hyperlink auditing lets sites report your clicks to
  // third parties. Off by default (equivalent to always-on --no-pings).
...
bool ChromeContentBrowserClient::CanCreateWindow(
  ...
  *no_javascript_access = false;
  ...
  // Zephyrus: block ad/scam pop-ups and pop-unders (the "click anywhere and a
  // new tab opens an ad site" behavior on sketchy sites).
  if (auto* zephyrus_adblock_service =
          zephyrus_adblock::ZephyrusAdblockServiceFactory::GetForBrowserContext(
              profile);
      zephyrus_adblock_service &&
      // Respect the per-site allowlist: a page the user allowlisted has ALL
      // blocking (network, cosmetic, scriptlet, and pop-up) suppressed.
      !zephyrus_adblock_service->IsAllowlisted(opener_top_level_frame_url)) {
    // Filter-based: destination matches a $popup or general block rule.
    if (zephyrus_adblock_service->ShouldBlockPopup(target_url, opener_url)) {
      return false;
    }
    // Aggressive heuristic: a script-opened (window.open) pop-up/pop-under to a
    // different site than the page. Genuine <a target=_blank> link clicks use
    // the browser's own new-tab path (not this hook), so this mainly catches
    // scripted popups. Foreground tabs are spared to avoid breaking normal
    // "open in new tab" behavior.
    // Only applies to web-page openers: browser UI (chrome:// pages, e.g.
    // sign-in flows launched from settings) legitimately opens cross-site
    // popups and would always fail the same-site check below.
    if (zephyrus_adblock_service->aggressive_popup_blocking() &&
        opener_top_level_frame_url.SchemeIsHTTPOrHTTPS() &&
        (disposition == WindowOpenDisposition::NEW_POPUP ||
         disposition == WindowOpenDisposition::NEW_BACKGROUND_TAB) &&
        target_url.is_valid() && target_url.SchemeIsHTTPOrHTTPS() &&
        !net::registry_controlled_domains::SameDomainOrHost(
            target_url, opener_top_level_frame_url,
            net::registry_controlled_domains::INCLUDE_PRIVATE_REGISTRIES)) {
      zephyrus_adblock_service->RecordBlockedPopup();
      return false;
    }
  }
...
// - http://crbug.com/40337093, http://crbug.com/41304476.
  ...
  // Zephyrus: Inter is the default web-content font. A page that specifies its
  // own font-family still wins (this only sets the fallback the browser uses
  // when a page relies on the generic families), so ordinary sites are
  // unaffected while default/unstyled content renders in Inter to match the
  // rest of the browser. Only the common-script ("Zyyy") entry is forced;
  // per-script entries (CJK, etc.) keep their language-appropriate defaults,
  // and `fixed` stays monospaced since Inter is not a monospace face.
  const std::u16string kZephyrusInter = u"Inter";
...
void ChromeContentBrowserClient::WillCreateURLLoaderFactory(
  ...
  // Zephyrus: native ad/tracker blocking. Proxying the factory here (rather than
  // a URL-loader throttle) is what lets us see subresource requests — scripts,
  // images, XHR — where nearly all ads/trackers live, not just navigations.
  zephyrus_adblock::ZephyrusAdblockProxyingURLLoaderFactory::MaybeProxyRequest(
