...
std::unique_ptr<content::WebContents> CreateTargetContents(
  ...
  // ZEPHYRUS: a tab opened with NO opener must still land in the current
  // workspace's StoragePartition.
  //
  // This is the ONLY place a workspace's cookies can leak into the shared jar.
  // The opener branch below is already correct for free -- a tab opened FROM a
  // workspace tab inherits its opener's SiteInstance, and the partition rides
  // along. But the no-opener branch (the new-tab button, an omnibox
  // navigation, a bookmark) builds against the DEFAULT partition, so without
  // this a user who typed a URL while in workspace 3 would be browsing with
  // workspace 1's session and nothing on screen would say so.
  scoped_refptr<content::SiteInstance> zephyrus_workspace_site_instance;
  if (!params.opener && params.browser) {
    Browser* zephyrus_browser = params.browser->GetBrowserForMigrationOnly();
    Profile* zephyrus_profile = params.browser->GetProfile();
    // Keyed on the Browser, which is what SetWindowWorkspace() records.
    if (zephyrus_browser && zephyrus_profile) {
      if (auto* store =
              ZephyrusWorkspaceStore::GetForProfile(zephyrus_profile)) {
        zephyrus_workspace_site_instance = zephyrus::SiteInstanceForWorkspace(
            zephyrus_profile, store->PartitionNameForWindow(zephyrus_browser),
            url);
      }
    }
  }
  ...
  scoped_refptr<content::SiteInstance> initial_site_instance_for_new_contents =
      params.opener ? params.opener->GetSiteInstance()
                    : (zephyrus_workspace_site_instance
                           ? zephyrus_workspace_site_instance
