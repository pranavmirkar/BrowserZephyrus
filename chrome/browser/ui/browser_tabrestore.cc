...
std::unique_ptr<WebContents> CreateRestoredTab(
  ...
  // ZEPHYRUS: a restored tab must come back in ITS WORKSPACE's
  // StoragePartition, not the default one.
  //
  // A tab's partition is fixed when the WebContents is created and cannot be
  // changed afterwards, so this cannot be left to the workspace tagging that
  // happens later at insertion. Without it, every restart silently returns
  // every workspace to the shared cookie jar -- isolation that lasts until you
  // close the browser is not isolation.
  //
  // The workspace comes from the same saved per-tab list that the tagging uses;
  // it is peeked here and consumed there, so one queue serves both.
  scoped_refptr<content::SiteInstance> zephyrus_site_instance;
  if (ZephyrusWorkspaceStore* zephyrus_store =
          ZephyrusWorkspaceStore::GetForProfile(browser->profile())) {
    // Two callers reach here and they need different answers.
    //
    // SESSION RESTORE has a saved per-tab workspace list, and it is
    // authoritative: those tabs belong to the workspaces they were in, which
    // may not be the one on screen while the window fills up.
    //
    // REOPENING A CLOSED TAB (Ctrl+Shift+T, the recently-closed menu) has no
    // such list -- it is only populated at session load. Falling through to the
    // default partition there would mean reopening a tab from an isolated
    // workspace and finding yourself signed in as the other account. The tab is
    // coming back into the window in front of you, so the window's current
    // workspace is the right answer.
    // Three sources, most specific first.
    //
    // 1. The closed tab's own record. Set when the tab was closed, so it
    //    survives reopening a tab OR a whole window, including into a Browser
    //    that was created a moment ago and knows nothing yet.
    std::string zephyrus_partition;
    if (auto it = extra_data.find(zephyrus::kWorkspaceExtraDataKey);
        it != extra_data.end()) {
      int zephyrus_workspace = 0;
      if (base::StringToInt(it->second, &zephyrus_workspace)) {
        zephyrus_partition =
            zephyrus_store->PartitionNameForWorkspace(zephyrus_workspace);
      }
    }
    // 2. The saved per-tab list, which only exists during session restore.
    if (zephyrus_partition.empty()) {
      zephyrus_partition = zephyrus_store->PartitionNameForPendingRestore();
    }
    // 3. The window the tab is landing in. Last, because it is a guess: right
    //    for a tab coming back into the window in front of you, wrong for a
    //    restored window that has not been assigned a workspace yet.
    if (zephyrus_partition.empty()) {
      zephyrus_partition = zephyrus_store->PartitionNameForWindow(browser);
    }
    zephyrus_site_instance = zephyrus::SiteInstanceForWorkspace(
        browser->profile(), zephyrus_partition, restore_url);
  }

  WebContents::CreateParams create_params(
      browser->profile(),
      zephyrus_site_instance
          ? zephyrus_site_instance
