...
HatsService::LaunchError HatsServiceDesktop::LaunchSurvey(
  ...
  // Zephyrus: no "Help us improve" satisfaction surveys. Reported as a missing
  // trigger config, which is the honest answer — this build ships no survey
  // configuration at all. The failure callback still runs so callers that wait
  // on a result are not left hanging.
  //
  // Body removed rather than early-returned: -Wunreachable-code is an error.
