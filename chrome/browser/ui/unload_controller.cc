...
void UnloadController::TabStripEmpty() {
  // Zephyrus: an empty tab strip no longer means the window is closing — a
  // normal window stays open on the empty backdrop (see Browser::TabStripEmpty).
  //
  // Inferring "closing" from emptiness here is what made closing the last tab
  // *sometimes* still close the browser: this observer can run before
  // Browser::TabStripEmpty(), whose guard then saw IsAttemptingToCloseBrowser()
  // and closed the window; and once any beforeunload handler finished,
  // ProcessPendingTabs() went on to call Browser::OnWindowClosing(). Whether it
  // happened depended on the page having unload handlers, hence "sometimes".
  //
  // Genuine closes are unaffected: they set this flag in ShouldCloseWindow() /
  // TryToCloseWindow() before any tab is closed.
