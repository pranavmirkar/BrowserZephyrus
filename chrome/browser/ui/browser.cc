...
void Browser::OnWindowClosing() {
  ...
  // Zephyrus: from here the window really is going away, so TabStripEmpty()
  // must let it close instead of holding it open on the empty state.
  zephyrus_window_closing_ = true;
...
void Browser::TabStripEmpty() {
  // Zephyrus: there is no new-tab page to fall back to, so closing the last tab
  // leaves a normal window open on the empty state (blurred wallpaper) rather
  // than destroying it — the user reopens with Ctrl+T. Genuine closes (the X,
  // Ctrl+Shift+W, profile or browser shutdown) run OnWindowClosing() first and
  // still fall through. Popups, apps and devtools keep stock behavior, since
  // they have no empty state and are expected to disappear with their content.
  if (is_type_normal() && !zephyrus_window_closing_ && !is_delete_scheduled_ &&
