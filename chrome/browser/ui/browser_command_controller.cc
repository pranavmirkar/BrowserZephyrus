...
bool BrowserCommandController::IsReservedCommandOrKey(
  ...
  return command_id == IDC_CLOSE_TAB || command_id == IDC_CLOSE_WINDOW ||
         ...
         command_id == IDC_ZEPHYRUS_WORKSPACE_NEXT ||
         command_id == IDC_ZEPHYRUS_WORKSPACE_PREV ||
         (command_id >= IDC_ZEPHYRUS_WORKSPACE_1 &&
          command_id <= IDC_ZEPHYRUS_WORKSPACE_8);
}
...
void BrowserCommandController::HandleCommandWithDisposition(
  ...
  switch (id) {
    ...
    case IDC_CLOSE_TAB: {
      ...
      CloseTab(browser_);
      // Zephyrus: keep the active tab inside the current workspace.
      if (BrowserView* browser_view =
              BrowserView::GetBrowserViewForBrowser(browser_)) {
        if (ZephyrusWorkspaceManager* workspace_manager =
                browser_view->zephyrus_workspace_manager()) {
          workspace_manager->EnsureActiveTabInWorkspace();
        }
      }
      break;
    }
    case IDC_SELECT_NEXT_TAB: {
      ...
      if (BrowserView* browser_view =
              BrowserView::GetBrowserViewForBrowser(browser_)) {
        workspace_manager = browser_view->zephyrus_workspace_manager();
      }
      if (workspace_manager) {
        workspace_manager->SelectAdjacentTabInWorkspace(1);
      } else {
    ...
    case IDC_MOVE_TAB_PREVIOUS:
      ...
      break;
    // Zephyrus: workspace switching.
    case IDC_ZEPHYRUS_WORKSPACE_NEXT:
    case IDC_ZEPHYRUS_WORKSPACE_PREV:
    case IDC_ZEPHYRUS_WORKSPACE_1:
    case IDC_ZEPHYRUS_WORKSPACE_2:
    case IDC_ZEPHYRUS_WORKSPACE_3:
    case IDC_ZEPHYRUS_WORKSPACE_4:
    case IDC_ZEPHYRUS_WORKSPACE_5:
    case IDC_ZEPHYRUS_WORKSPACE_6:
    case IDC_ZEPHYRUS_WORKSPACE_7:
    case IDC_ZEPHYRUS_WORKSPACE_8: {
      BrowserView* browser_view =
          BrowserView::GetBrowserViewForBrowser(browser_);
      ZephyrusWorkspaceManager* workspace_manager =
          browser_view ? browser_view->zephyrus_workspace_manager() : nullptr;
      if (workspace_manager) {
        if (id == IDC_ZEPHYRUS_WORKSPACE_NEXT) {
          workspace_manager->SwitchToAdjacentWorkspace(1);
        } else if (id == IDC_ZEPHYRUS_WORKSPACE_PREV) {
          workspace_manager->SwitchToAdjacentWorkspace(-1);
        } else {
          workspace_manager->SwitchToWorkspaceByIndex(
              static_cast<size_t>(id - IDC_ZEPHYRUS_WORKSPACE_1));
        }
      }
      break;
    }
    ...
    case IDC_SHOW_BOOKMARK_MANAGER:
      // Zephyrus: centralized settings popup (falls back for popups/OTR).
      if (ZephyrusSettingsPopup::MaybeShow(
              browser_, ZephyrusSettingsPopup::Section::kBookmarks)) {
        break;
      }
    ...
    case IDC_SHOW_HISTORY:
      // Zephyrus: centralized settings popup (falls back for popups/OTR).
      if (ZephyrusSettingsPopup::MaybeShow(
              browser_, ZephyrusSettingsPopup::Section::kHistory)) {
        break;
      }
    ...
    case IDC_SHOW_DOWNLOADS:
      // Zephyrus: centralized settings popup (falls back for popups/OTR).
      if (ZephyrusSettingsPopup::MaybeShow(
              browser_, ZephyrusSettingsPopup::Section::kDownloads)) {
        break;
      }
    ...
    case IDC_EXTENSIONS_SUBMENU_MANAGE_EXTENSIONS:
      // Zephyrus: centralized settings popup (falls back for popups/OTR).
      if (ZephyrusSettingsPopup::MaybeShow(
              browser_, ZephyrusSettingsPopup::Section::kExtensions)) {
        break;
      }
    ...
    case IDC_OPTIONS:
      // Zephyrus: centralized settings popup (falls back for popups/OTR).
      if (ZephyrusSettingsPopup::MaybeShow(
              browser_, ZephyrusSettingsPopup::Section::kSettings)) {
        break;
      }
    ...
    case IDC_ABOUT:
      // Zephyrus: centralized settings popup (falls back for popups/OTR).
      if (ZephyrusSettingsPopup::MaybeShow(
              browser_, ZephyrusSettingsPopup::Section::kAbout)) {
        break;
      }
...
void BrowserCommandController::InitCommandState() {
  ...
  command_updater_->UpdateCommandEnabled(IDC_SELECT_PREVIOUS_TAB,
                                         supports_tabs);
  // Zephyrus: workspace switching (enabled whenever the window has a tab strip).
  command_updater_->UpdateCommandEnabled(IDC_ZEPHYRUS_WORKSPACE_NEXT,
                                         supports_tabs);
  command_updater_->UpdateCommandEnabled(IDC_ZEPHYRUS_WORKSPACE_PREV,
                                         supports_tabs);
  for (int cmd = IDC_ZEPHYRUS_WORKSPACE_1; cmd <= IDC_ZEPHYRUS_WORKSPACE_8;
       ++cmd) {
    command_updater_->UpdateCommandEnabled(cmd, supports_tabs);
  }
