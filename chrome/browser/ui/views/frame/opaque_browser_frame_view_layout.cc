...
gfx::Rect OpaqueBrowserFrameViewLayout::CalculateClientAreaBounds(
  ...
  int top_height;
  if (zephyrus_titlebar_mode_) {
    // The client area (toolbar + contents) fills the entire window with no
    // frame border on any side, so the toolbar is flush with the window edges
    // and the title bar is the very top of the window.
    border_thickness = gfx::Insets();
    top_height = 0;
  } else if (is_window_controls_overlay_enabled_ ||
             is_unframed_mode_enabled_ ||
             delegate_->WebAppButtonHeight() > 0) {
    top_height = border_thickness.top();
  } else {
    top_height = NonClientTopHeight(false);
  }
  return gfx::Rect(
...
void OpaqueBrowserFrameViewLayout::LayoutWindowControls() {
  ...
  // Zephyrus: the toolbar hosts its own window controls, so hide the frame's
  // caption buttons entirely.
  if (zephyrus_titlebar_mode_) {
