...
BrowserView::BrowserView(Browser* browser)
  ...
  // Zephyrus: floating tab/workspace sidebar, overlaying the left edge of the
  // content area for normal tabbed windows. The hot-zone is added first (lower
  // z-order) so the revealed sidebar paints/handles input above it.
  if (browser_->is_type_normal()) {
    zephyrus_workspace_manager_ =
        std::make_unique<ZephyrusWorkspaceManager>(browser_.get());
    zephyrus_sidebar_hotzone_ =
        AddChildView(std::make_unique<ZephyrusSidebarHotZone>(
            base::BindRepeating(
                [](BrowserView* browser_view) {
                  if (browser_view->zephyrus_sidebar_) {
                    browser_view->zephyrus_sidebar_->Reveal();
                  }
                },
...
bool BrowserView::ShouldDrawTabStrip() const {
  // Zephyrus: the horizontal tab strip is replaced by the floating sidebar
  // (Phase 4), so it is never drawn for normal tabbed browser windows. Tabs
  // remain fully functional via the TabStripModel; only the horizontal strip
  // view is hidden. Popups/apps/devtools are unaffected.
  if (browser_->is_type_normal() && horizontal_tab_strip_region_view_ &&
...
void BrowserView::OnActiveTabChanged(content::WebContents* old_contents,
  ...
  // Zephyrus: tint the title bar with the newly active page's color, and sync
...
void BrowserView::FadeOutZephyrusScrim(base::TimeDelta duration) {
  if (!window_scrim_view_ || !window_scrim_view_->GetVisible() ||
      !window_scrim_view_->layer()) {
    return;
  }
  if (!gfx::Animation::ShouldRenderRichAnimation()) {
    window_scrim_view_->layer()->SetOpacity(0.0f);
    return;
  }
  ui::ScopedLayerAnimationSettings settings(
      window_scrim_view_->layer()->GetAnimator());
...
void BrowserView::OnWidgetWindowModalVisibilityChanged(views::Widget* widget,
  ...
  // Zephyrus: fade the scrim in rather than snapping, so it arrives with the
  // dialog's own 200ms entrance instead of flashing ahead of it. Only the
  // entrance is animated — on dismissal the dialog vanishes at once, so a
  // lingering scrim would be the thing that looks wrong.
  if (visible && window_scrim_view_->layer() &&
    ...
    window_scrim_view_->layer()->SetOpacity(1.0f);
  } else if (window_scrim_view_->layer()) {
    window_scrim_view_->layer()->SetOpacity(1.0f);
  }
#endif
}
...
void BrowserView::DidFinishNavigation(
  ...
  // Zephyrus: a navigation may change the page color; refresh the title bar.
  UpdateZephyrusTitlebarColor();
}
...
void BrowserView::OnBackgroundColorChanged() {
  UpdateZephyrusTitlebarColor();
}

void BrowserView::DidFirstVisuallyNonEmptyPaint() {
  // The page's background color is reliable once it has painted.
  UpdateZephyrusTitlebarColor();
}
...
void BrowserView::DidStopLoading() {
  if (LocationBarView* location_bar = GetLocationBarView()) {
    location_bar->SetZephyrusLoading(false);
  }
}

SkColor BrowserView::GetZephyrusThemeColor() const {
  // Private Workspace wears a different base color, and because every Zephyrus
  // surface derives from this one value, changing it here recolors the title
...
void BrowserView::UpdateZephyrusTitlebarColor() {
  if (!browser_->is_type_normal() || !toolbar_) {
    return;
  }
  // Zephyrus: PERMANENT theme. The chrome no longer adapts to the active page
  // and does not switch between light and dark — every surface derives from the
  // one fixed base below, so the browser looks identical on every site.
  const std::optional<SkColor> color = GetZephyrusThemeColor();
  // This runs on every navigation/paint signal (including subframes), so skip
  // the downstream rebuilds (toolbar backgrounds, omnibox refresh, sidebar
  // recolor) when the effective color did not actually change.
  ...
  // rounded top corners of the web contents (the notch), making the title bar
  // appear to curve into the page. The container view can differ per tab, so
  // refresh it when either the color or the active container changes.
  if (ContentsContainerView* contents_container =
...
void BrowserView::ToggleZephyrusTitlebarPinned() {
  ...
  if (zephyrus_titlebar_pinned_) {
    ...
    zephyrus_titlebar_anim_timer_.Stop();
    zephyrus_titlebar_anim_t_ = 1.0;
    zephyrus_titlebar_anim_target_ = 1.0;
    InvalidateLayout();
  } else {
    // Poll the cursor so the title bar can reveal on a top-edge hover (move
    // events don't reliably reach views sitting over web contents), and slide
    // the title bar out now.
...
void BrowserView::OnZephyrusTitlebarRevealPoll() {
  if (zephyrus_titlebar_pinned_ || !GetWidget()) {
    return;
  }
  // Nothing to reveal/hide when the window isn't on screen; skip the cursor
  // and display queries entirely.
  if (GetWidget()->IsMinimized() || !GetWidget()->IsVisible()) {
    return;
  }
  const gfx::Point cursor = display::Screen::Get()->GetCursorScreenPoint();
  // Measure from the window's *visible* top edge. A maximized window extends a
  // few pixels off-screen (negative y), so a band tied to window.y() would sit
  // above the screen and never be reachable; intersect with the work area.
  gfx::Rect visible = GetWidget()->GetWindowBoundsInScreen();
  const gfx::Rect work_area =
      display::Screen::Get()
          ->GetDisplayNearestWindow(GetWidget()->GetNativeWindow())
          .work_area();
  visible.Intersect(work_area);
  if (visible.IsEmpty()) {
    visible = GetWidget()->GetWindowBoundsInScreen();
  }
  const bool cursor_in_x =
      cursor.x() >= visible.x() && cursor.x() < visible.right();

  if (zephyrus_titlebar_anim_target_ < 1.0) {
    // Hidden/hiding: reveal when the pointer reaches the very top edge.
    constexpr int kTopZone = 6;
    if (GetWidget()->IsActive() && cursor_in_x && cursor.y() >= visible.y() &&
        cursor.y() < visible.y() + kTopZone) {
      StartZephyrusTitlebarAnim(1.0);
    }
    return;
  }

  // Shown/showing: keep it while the pointer is over the title bar or the
  // toolbar holds focus (e.g. typing in the omnibox); otherwise hide it.
  const int reveal_bottom = (toolbar_ && toolbar_->GetVisible())
                                ? toolbar_->GetBoundsInScreen().bottom()
                                : visible.y() + 40;
  const views::View* focused =
      GetFocusManager() ? GetFocusManager()->GetFocusedView() : nullptr;
  const bool toolbar_focused =
      toolbar_ && focused && toolbar_->Contains(focused);
  if (!toolbar_focused && (!cursor_in_x || cursor.y() > reveal_bottom)) {
    StartZephyrusTitlebarAnim(0.0);
  }
}

void BrowserView::StartZephyrusTitlebarAnim(double target) {
  zephyrus_titlebar_anim_target_ = target;
  if (zephyrus_titlebar_anim_t_ == target) {
    return;
  }
  if (!zephyrus_titlebar_anim_timer_.IsRunning()) {
    zephyrus_titlebar_anim_timer_.Start(
        FROM_HERE, base::Milliseconds(15), this,
        &BrowserView::OnZephyrusTitlebarAnimTick);
  }
}

void BrowserView::OnZephyrusTitlebarAnimTick() {
  // ~15ms tick over a ~165ms slide.
  constexpr double kStep = 15.0 / 165.0;
  if (zephyrus_titlebar_anim_t_ < zephyrus_titlebar_anim_target_) {
    zephyrus_titlebar_anim_t_ =
        std::min(zephyrus_titlebar_anim_target_, zephyrus_titlebar_anim_t_ + kStep);
  } else {
    zephyrus_titlebar_anim_t_ =
        std::max(zephyrus_titlebar_anim_target_, zephyrus_titlebar_anim_t_ - kStep);
  }
  if (zephyrus_titlebar_anim_t_ == zephyrus_titlebar_anim_target_) {
    zephyrus_titlebar_anim_timer_.Stop();
  }
...
double BrowserView::ZephyrusTitlebarRevealRatio() const {
  return gfx::Tween::CalculateValue(gfx::Tween::EASE_IN_OUT,
                                    zephyrus_titlebar_anim_t_);
}

void BrowserView::ApplyZephyrusTitlebarReveal() {
  if (zephyrus_titlebar_pinned_ || !top_container_ || !contents_container_) {
    return;
  }
  const double ratio = ZephyrusTitlebarRevealRatio();
  if (ratio >= 1.0) {
    return;  // Fully shown: laid out normally.
  }
  const int shift =
      base::ClampRound((1.0 - ratio) * top_container_->height());
  if (shift <= 0) {
    return;
  }
  // Slide the title bar up off the top edge...
  gfx::Rect top_bounds = top_container_->bounds();
  top_bounds.set_y(top_bounds.y() - shift);
  top_container_->SetBoundsRect(top_bounds);
  // ...and pull the contents up to follow its bottom edge, growing to fill the
  // freed space so the page reflows smoothly rather than snapping.
  gfx::Rect contents_bounds = contents_container_->bounds();
  contents_bounds.set_y(contents_bounds.y() - shift);
  contents_bounds.set_height(contents_bounds.height() + shift);
  contents_container_->SetBoundsRect(contents_bounds);
}
...
bool BrowserView::IsToolbarVisible() const {
  ...
  // Zephyrus: the toolbar doubles as the title bar for normal windows; when it
  // is unpinned and not currently revealed by a top-edge hover, hide it so the
  // web page fills the whole window.
  if (browser_->is_type_normal() && !IsZephyrusTitlebarShowing()) {
    return false;
  }
...
int BrowserView::NonClientHitTest(const gfx::Point& point) {
  ...
  // Zephyrus: the toolbar doubles as the title bar for normal windows. Empty
  // toolbar areas (not over an interactive control) are draggable caption.
  if (browser_->is_type_normal() && !GetTabStripVisible() && toolbar_ &&
      toolbar_->GetVisible()) {
    gfx::Point toolbar_point(point);
    views::View::ConvertPointToTarget(parent(), toolbar_, &toolbar_point);
    if (toolbar_->HitTestPoint(toolbar_point)) {
      views::View* handler = toolbar_->GetEventHandlerForPoint(toolbar_point);
      return (handler == toolbar_) ? HTCAPTION : HTCLIENT;
    }
  }
