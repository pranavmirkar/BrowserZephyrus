...
class CustomCornersBackground : public views::Background, public CustomCorners {
 public:
  ...
  // Zephyrus: when set, the primary area is painted with this explicit color
  // instead of resolving `primary_color_`. Used to tint the title bar with the
  // active web page's color. Pass std::nullopt to fall back to the theme color.
  void SetExplicitPrimaryColor(std::optional<SkColor> color);
 ...
 private:
  ...
  std::optional<SkColor> explicit_primary_color_;
