...
class BrowserDesktopWindowTreeHostWin
 ...
 private:
  ...
  // Zephyrus: rounds the window corners (Win11) only when restored; uses square
  // corners when fullscreen/maximized so no wallpaper shows through.
  void UpdateWindowCornerPreference();
  // Last corner preference applied, so WM_WINDOWPOSCHANGED (which fires
  // continuously during drags/resizes) doesn't issue redundant DWM calls.
  int last_corner_preference_ = -1;
