...
bool BrowserDesktopWindowTreeHostWin::GetClientAreaInsets(
  ...
  if (GetWidget()->IsFullscreen()) {
    ...
    *insets = gfx::Insets();
  } else if (GetWidget()->IsMaximized()) {
    // Maximized: Windows outdents the window rect from the monitor's client
    // rect by |frame_thickness| on each edge so the resize border is hidden
    // off-screen.  We must echo that here or the client area bleeds onto
    // adjacent monitors.
    int top_thickness = ShouldBrowserCustomDrawTitlebar(browser_view_)
                            ? frame_thickness : 0;
    *insets = gfx::Insets::TLBR(top_thickness, frame_thickness,
                                 frame_thickness, frame_thickness);
  } else {
    // Zephyrus frameless: eliminate all native non-client borders.  Resize
    // hit-testing is handled in BrowserFrameViewWin::NonClientHitTest(); the
    // DWM shadow is preserved because WS_CAPTION remains in the window style.
    *insets = gfx::Insets();
  }
  return true;
}
...
void BrowserDesktopWindowTreeHostWin::HandleCreate() {
  ...
  // Zephyrus: the custom frame doesn't opt into Chromium's rounded-corner path,
  // so request Win11 rounded corners explicitly for a polished look (square
  // corners when fullscreen/maximized — see UpdateWindowCornerPreference).
...
bool BrowserDesktopWindowTreeHostWin::PreHandleMSG(UINT message,
  ...
  switch (message) {
    case WM_NCHITTEST: {
      // Zephyrus frameless: the non-client area is zero on all sides so
      // Windows cannot provide resize hit-testing natively.  We must return
      // the correct HT* code before Views processes the message, because the
      // Views NonClientHitTest path returns HTCLIENT for content-area pixels
      // before it ever reaches the resize-border check.
      if (!GetWidget()->IsMaximized() && !GetWidget()->IsFullscreen()) {
        HWND hwnd = GetHWND();
        POINT screen_pt = {static_cast<LONG>(LOWORD(l_param)),
                           static_cast<LONG>(HIWORD(l_param))};
        POINT client_pt = screen_pt;
        ::ScreenToClient(hwnd, &client_pt);
        RECT rc;
        ::GetClientRect(hwnd, &rc);
        const int cx = client_pt.x;
        const int cy = client_pt.y;
        const int w  = rc.right;
        const int h  = rc.bottom;
        // Scale the logical resize border (6 DIP) to physical pixels.
        const int dpi    = ::GetDpiForWindow(hwnd);
        const int border = ::MulDiv(6, dpi, 96);
        const bool on_left   = cx < border;
        const bool on_right  = cx >= w - border;
        const bool on_top    = cy < border;
        const bool on_bottom = cy >= h - border;
        if (on_top    && on_left)  { *result = HTTOPLEFT;    return true; }
        if (on_top    && on_right) { *result = HTTOPRIGHT;   return true; }
        if (on_bottom && on_left)  { *result = HTBOTTOMLEFT; return true; }
        if (on_bottom && on_right) { *result = HTBOTTOMRIGHT; return true; }
        if (on_left)               { *result = HTLEFT;       return true; }
        if (on_right)              { *result = HTRIGHT;      return true; }
        if (on_top)                { *result = HTTOP;        return true; }
        if (on_bottom)             { *result = HTBOTTOM;     return true; }
      }
      break;
    }
    case WM_ENDSESSION:
...
void BrowserDesktopWindowTreeHostWin::PostHandleMSG(UINT message,
  ...
  switch (message) {
    ...
    case WM_WINDOWPOSCHANGED: {
      ...
      // Zephyrus: refresh rounded vs. square corners on size/state changes
      // (entering/exiting fullscreen or maximize).
      UpdateWindowCornerPreference();
      break;
    }
...
void BrowserDesktopWindowTreeHostWin::UpdateWindowCornerPreference() {
  if (base::win::GetVersion() < base::win::Version::WIN11 || !GetWidget()) {
    return;
  }
  // Fullscreen/maximized fill the screen edge-to-edge, so rounded corners would
  // reveal the wallpaper behind them. Only round restored windows.
  const bool square = GetWidget()->IsFullscreen() || GetWidget()->IsMaximized();
  DWM_WINDOW_CORNER_PREFERENCE corner_pref =
      square ? DWMWCP_DONOTROUND : DWMWCP_ROUND;
  // Skip redundant DWM calls: this runs on every WM_WINDOWPOSCHANGED, which
  // fires continuously while the window is dragged or resized.
  if (static_cast<int>(corner_pref) == last_corner_preference_) {
    return;
  }
  last_corner_preference_ = static_cast<int>(corner_pref);
