...
void MultiContentsView::UpdateContentsBorderAndOverlay() {
  ...
  // Only the side-by-side layout ships (kSplitViewHorizontal is off), so the
  // facing edges are leading/trailing. A stacked layout would need the same
  // treatment on top/bottom.
  int pane = 0;
  for (auto* contents_container_view : contents_container_views_) {
    contents_container_view->SetZephyrusSuppressedEdge(
        !is_in_split ? 0 : (pane == 0 ? 2 : 1));
    ++pane;
    const bool is_active =
        contents_container_view->contents_view() == GetActiveContentsView();
    // Zephyrus: split panes keep the same corners as everything else. Upstream
    // rounds them at 6 while the content card uses 8 -- close enough to look
    // like a mistake rather than a choice, which is the worst of both.
    contents_container_view->SetRoundedCorners(
        is_in_split
            ? gfx::RoundedCornersF(ContentsContainerOutline::kCornerRadius)
