...
class OpaqueBrowserFrameViewLayout : public views::LayoutManager {
 public:
  ...
  // Zephyrus: when enabled, the client area (toolbar + contents) extends to the
  // very top of the window so the toolbar becomes the title bar. The frame's
  // own caption buttons are hidden; window controls live in the toolbar.
  void SetZephyrusTitlebarMode(bool enabled) {
    zephyrus_titlebar_mode_ = enabled;
  }
  bool zephyrus_titlebar_mode() const { return zephyrus_titlebar_mode_; }
 ...
 private:
  ...
  bool zephyrus_titlebar_mode_ = false;
