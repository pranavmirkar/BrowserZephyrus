...
OpaqueBrowserFrameView::OpaqueBrowserFrameView(
  ...
  // Zephyrus: for normal tabbed browser windows, extend the toolbar to the top
  // of the window so it becomes the title bar (with window controls hosted in
  // the toolbar itself). The horizontal tab strip is already hidden.
  if (browser_view->browser()->is_type_normal()) {
    layout_->SetZephyrusTitlebarMode(true);
  }
...
int OpaqueBrowserFrameView::NonClientHitTest(const gfx::Point& point) {
  // Zephyrus: the native non-client border is zero on Windows, so the client
  // view's NonClientHitTest would return HTCLIENT for edge pixels before the
  // existing resize-border check at the bottom of this function is reached.
  // Detect resize edges first so they are never swallowed by HTCLIENT.
#if BUILDFLAG(IS_WIN)
  if (!IsFrameCondensed() && !browser_widget()->IsFullscreen() &&
      browser_widget()->widget_delegate()->CanResize()) {
    constexpr int kResizeBorder = 6;
    const int w = width();
    const int h = height();
    const bool on_left   = point.x() < kResizeBorder;
    const bool on_right  = point.x() >= w - kResizeBorder;
    const bool on_top    = point.y() < kResizeBorder;
    const bool on_bottom = point.y() >= h - kResizeBorder;
...
OpaqueBrowserFrameView::GetCaptionButtonBounds() const {
  // Zephyrus: the toolbar is the title bar and hosts its own window controls,
  // so the frame reserves no caption-button exclusion. Returning empty bounds
  // lets the toolbar occupy the very top of the window.
  if (layout_->zephyrus_titlebar_mode()) {
