...
class BrowserWidget : public views::Widget,
 ...
 public:
  ...
  // Zephyrus: this window's own theme, for when it shows a workspace other than
  // the one whose look the profile's theme prefs currently hold (see
  // ZephyrusWorkspaceStore::MirrorAppearance). nullopt follows the profile.
  //
  // Not Widget::SetUserColorOverride(): that can only name a colour, and a
  // workspace on Zephyrus's DEFAULT palette has none -- the default is a
  // hand-tuned ramp chosen by the absence of a user colour, which only a key
  // built here can express.
  struct ZephyrusLook {
    std::optional<SkColor> seed;
    bool dark = true;
    bool grayscale = false;
    bool operator==(const ZephyrusLook&) const = default;
  };
  void SetZephyrusLook(std::optional<ZephyrusLook> look);
 ...
 private:
  ...
  std::optional<ZephyrusLook> zephyrus_look_;
