...
int BrowserFrameViewWin::NonClientHitTest(const gfx::Point& point) {
  // Zephyrus frameless: the native non-client border is zero for restored
  // windows, so we must detect resize hits manually before any other check.
  if (!IsMaximized() && !browser_widget()->IsFullscreen() &&
