...
class BrowserView : public BrowserWindow,
 ...
 public:
  ...
  SkColor GetZephyrusThemeColor() const;
  ...
  // Zephyrus: fades the window-modal scrim out over `duration`, leaving the
  // view visible. A modal that plays its own exit animation calls this when it
  // starts, so the scrim lifts together with the dialog instead of waiting for
  // the widget to actually close. The stock
  // OnWidgetWindowModalVisibilityChanged(false) still hides the view for real
  // afterwards; by then its opacity is already 0, so nothing flashes.
  void FadeOutZephyrusScrim(base::TimeDelta duration);
  ...
  // Zephyrus: updates the title bar tint to match the active web page's color.
  void UpdateZephyrusTitlebarColor();

  // Zephyrus: the active page's color used for dynamic theming (nullopt when it
  // can't be determined). Consumed by the dynamically-themed popups (downloads,
  // app menu) so they adapt to the page like the title bar/omnibox/sidebar.
  std::optional<SkColor> GetZephyrusPageColor() const {
    return zephyrus_page_color_;
  }

  // Zephyrus: the workspace manager for this window (null for non-normal
  // windows).
  ZephyrusWorkspaceManager* zephyrus_workspace_manager() {
    return zephyrus_workspace_manager_.get();
  }
  ...
  void ToggleZephyrusTitlebarPinned();
  bool IsZephyrusTitlebarPinned() const { return zephyrus_titlebar_pinned_; }
  // Whether the title bar should currently be laid out (pinned, or at all
  // revealed/animating by a top-edge hover while unpinned).
  bool IsZephyrusTitlebarShowing() const {
    return zephyrus_titlebar_pinned_ || zephyrus_titlebar_anim_t_ > 0.0;
  }
 ...
 private:
  ...
  // the toolbar (title bar) is hidden via IsToolbarVisible() and a cursor poll
  // reveals it while the pointer is at the top edge. The reveal/hide is animated
  // by sliding the title bar in from the top while the page content reflows to
  ...
  base::RepeatingTimer zephyrus_titlebar_reveal_timer_;  // cursor poll
  base::RepeatingTimer zephyrus_titlebar_anim_timer_;    // reflow animation
  void OnZephyrusTitlebarRevealPoll();
  void OnZephyrusTitlebarAnimTick();
  void StartZephyrusTitlebarAnim(double target);
  // Eased reveal ratio (0..1) derived from `anim_t_`.
  double ZephyrusTitlebarRevealRatio() const;
  // Post-layout pass that slides the title bar in and reflows contents to match
  // the current reveal ratio (no-op when fully shown/pinned).
  void ApplyZephyrusTitlebarReveal();
