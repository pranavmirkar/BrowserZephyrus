...
void ContentsContainerOutline::OnPaint(gfx::Canvas* canvas) {
  ...
  // Zephyrus: an INVISIBLE mini toolbar counts as absent. Outside split view
  // the toolbar is hidden but keeps its last laid-out size, and reading that
  // size regardless would cut a notch in the bottom-right of the frame for a
  // control that is not on screen.
  gfx::SizeF mini_toolbar_size;
  const bool hide_mini_toolbar =
      (is_active_ && is_highlighted_) ||
      !(mini_toolbar_ && mini_toolbar_->GetVisible());
