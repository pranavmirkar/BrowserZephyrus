...
void CustomCornersBackground::SetExplicitPrimaryColor(
    std::optional<SkColor> color) {
  if (explicit_primary_color_ == color) {
    return;
  }
  explicit_primary_color_ = color;
  view_->SchedulePaint();
}
...
void CustomCornersBackground::Paint(gfx::Canvas* canvas,
  ...
  if (explicit_primary_color_.has_value()) {
    // Zephyrus: paint the title bar with the active web page's color.
    cc::PaintFlags flags;
    flags.setAntiAlias(true);
    flags.setStyle(cc::PaintFlags::kFill_Style);
    flags.setColor(*explicit_primary_color_);
    canvas->DrawPath(background_path, flags);
  } else {
    PaintPath(canvas, background_path, primary_color_, /*anti_alias=*/true);
  }
