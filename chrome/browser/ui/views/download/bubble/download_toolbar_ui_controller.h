...
class DownloadToolbarUIController
 ...
 public:
  ...
  // Zephyrus: opens the downloads bubble anchored to `anchor` instead of to
  // the toolbar's own download button.
  //
  // The toolbar's button is ephemeral -- it exists only while there are recent
  // downloads, and it is inside the pinned-actions container, which stays on
  // the title bar. With the title bar hidden in compact mode there is often no
  // anchor at all, so ShowDetails() finds none and returns silently, which
  // reads as a dead button in the sidebar.
  //
  // Returns the bubble's widget, or null when none was shown: there is nothing
  // to display when the download list is empty, and the caller (whose button is
  // always on screen, unlike the toolbar's) needs to do something else in that
  // case. The widget is returned rather than a bool because a caller anchoring
  // into a panel that can slide away has to know when the bubble closes.
  // `arrow` says which edge of the anchor the bubble hangs off; the sidebar's
  // button is at the foot of the panel, so its bubble has to open upward.
  views::Widget* ZephyrusShowDetailsAnchoredTo(
      views::View* anchor,
 ...
 private:
  ...
  // Zephyrus: set for one call by ZephyrusShowDetailsAnchoredTo(), consumed by
  // OnBubbleAnchorAssembled(). The toolbar's own path leaves it empty and
  // keeps TOP_CENTER.
  std::optional<views::BubbleBorder::Arrow> zephyrus_arrow_;
