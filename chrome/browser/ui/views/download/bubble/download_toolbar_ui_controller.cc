...
void DownloadToolbarUIController::OnBubbleAnchorAssembled(
  ...
  // Zephyrus: tint the bubble with the active page's color so it participates in
  // the dynamic theme (like the title bar/omnibox/sidebar).
  const std::optional<SkColor> zephyrus_page_color =
      browser_view_ ? browser_view_->GetZephyrusPageColor() : std::nullopt;
  if (zephyrus_page_color) {
    bubble_delegate->SetBackgroundColor(*zephyrus_page_color);
  }
  ...
  // page-tinted background above, even though the browser chrome is forced dark.
  if (zephyrus_page_color) {
    bubble_widget->SetColorModeOverride(
        color_utils::IsDark(*zephyrus_page_color)
