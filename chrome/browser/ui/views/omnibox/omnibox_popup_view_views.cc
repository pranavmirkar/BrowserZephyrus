...
gfx::Rect OmniboxPopupViewViews::GetTargetBounds() const {
  ...
  // Zephyrus (Figma searchbar_resultdropdown): widen the popup so that, inside
  // the frame, there is exactly kZephyrusHeroOverhang of margin on each side of
  // the search bar. The card is then inset by that much — landing flush with
  // the bar — and the selected "hero" row overhangs into the margin (Figma:
  // hero 531 vs bar/card 477).
  const int zephyrus_extra_width =
      RoundedOmniboxResultsFrame::kZephyrusHeroOverhang -
      RoundedOmniboxResultsFrame::GetLocationBarAlignmentInsets().left();
  if (zephyrus_extra_width > 0) {
    content_rect.Inset(gfx::Insets::VH(0, -zephyrus_extra_width));
  }
