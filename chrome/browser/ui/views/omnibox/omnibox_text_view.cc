...
void ApplyTextStyleForAnswer(OmniboxResultView* result_view,
  ...
  render_text->ApplyColor(result_view->GetThemedColor(id), range);
}
...
void OmniboxTextView::ApplyTextColor(ui::ColorId id) {
  ...
  render_text_->SetColor(result_view_->GetThemedColor(id));
  SchedulePaint();
}
...
void OmniboxTextView::SetMultilineText(const std::u16string& text) {
  ...
  render_text_->SetColor(
      result_view_->GetThemedColor(kColorOmniboxResultsTextAnswer));
  render_text_->SetMultiline(true);
...
void OmniboxTextView::ReapplyStyling() {
  ...
  for (size_t i = 0; i < cached_classifications_->size(); ++i) {
    ...
    render_text_->ApplyColor(result_view_->GetThemedColor(id), current_range);
  }
