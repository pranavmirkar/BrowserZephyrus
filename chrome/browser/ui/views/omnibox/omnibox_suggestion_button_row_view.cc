...
// Zephyrus: resolve a popup colour the way the ROW resolves it.
//
// The popup's colours are page-adapted, so they cannot live in the global
// color provider -- OmniboxResultView asks the location bar for them instead.
// A button asking its color provider directly gets the stock Chromium value,
// which is how the tonal chip came out a flat grey while the row it sits on
// was tinted.
SkColor ZephyrusPopupColor(const views::View* view, ui::ColorId id) {
  for (const views::View* ancestor = view; ancestor;
       ancestor = ancestor->parent()) {
    if (const auto* row = views::AsViewClass<OmniboxResultView>(ancestor)) {
      return row->GetThemedColor(id);
    }
  }
  return view->GetColorProvider()->GetColor(id);
}
...
class OmniboxSuggestionRowButton : public views::MdTextButton {
 ...
 public:
  ...
  void OnThemeChanged() override {
    MdTextButton::OnThemeChanged();
    // We can't use colors from NativeTheme as the omnibox theme might be
    // different (for example, if the NTP colors are customized) -- and in
    // Zephyrus they are page-adapted, which is why these go through
    // ZephyrusPopupColor rather than the color provider.
    ...
    if (!image_.IsEmpty()) {
      ...
      SetImageModel(
          ...
          ui::ImageModel::FromVectorIcon(
              *icon_,
              ZephyrusPopupColor(this, selected
                                           ? kColorOmniboxResultsButtonIcon
                                           : kColorOmniboxResultsTextSelected),
              GetLayoutConstant(LayoutConstant::kLocationBarIconSize)));
    }
    // Zephyrus: the chip is filled with the tone the ROW is not, so its label
    // takes that tone's ink -- which is the opposite of the row's. See
    // UpdateBackgroundColor().
    SetEnabledTextColors(ZephyrusPopupColor(
        this, selected ? kColorOmniboxText : kColorOmniboxResultsTextSelected));
  ...
  void UpdateBackgroundColor() override {
    // Zephyrus: an M3 tonal chip, not an outlined button.
    //
    // It used to fill with its own ROW's background and draw a 1px outline,
    // which on a dark list makes a hole with a ring around it -- and an
    // outlined button is M3's loudest chip, spent here on "switch to this
    // tab", a secondary action sitting beside the thing you actually came for.
    //
    // The fill is the tone the row is NOT: the selection colour on an ordinary
    // row, the list's own surface on the selected one. That keeps the chip
    // legible in both states without inventing a third colour, and it stops
    // the chip disappearing into a selected row that is now itself tonal.
    const SkColor fill_color = ZephyrusPopupColor(
        this, theme_state_ == OmniboxPartState::SELECTED
                  ? kColorOmniboxResultsBackground
                  : kColorOmniboxResultsBackgroundSelected);
    SetBackground(CreateBackgroundFromPainter(
        views::Painter::CreateSolidRoundRectPainterWithVariableRadius(
            fill_color, GetCornerRadii(), gfx::Insets(),
