...
class TopBackgroundView : public views::View {
 ...
 public:
  TopBackgroundView(const LocationBar* location_bar,
                    std::optional<SkColor> zephyrus_background)
      : location_bar_(location_bar),
        zephyrus_background_(zephyrus_background) {}

  void OnThemeChanged() override {
    views::View::OnThemeChanged();
    const SkColor background_color = zephyrus_background_.value_or(
 ...
 private:
  ...
  std::optional<SkColor> zephyrus_background_;
};
...
RoundedOmniboxResultsFrame::RoundedOmniboxResultsFrame(
  ...
  // Zephyrus: if the title bar has adopted the page color, paint the results
  // panel in the matching color instead of the fixed theme color.
  std::optional<SkColor> zephyrus_background;
  if (auto* popup = views::AsViewClass<OmniboxPopupViewViews>(contents)) {
    if (LocationBarView* popup_location_bar = popup->location_bar_view()) {
      zephyrus_background = popup_location_bar->GetZephyrusOmniboxColor(
          kColorOmniboxResultsBackground);
    }
  }

  // Zephyrus (Figma searchbar_resultdropdown): the results surface is a
  // DETACHED light card floating below the (separate, pill-shaped) search
  // bar — not Chromium's unified bar+popup sheet. The card is painted inset:
  // clear of the bar region plus a gap on top, with slim transparent gutters
  // on the sides that the selected row's white pill "jumps" into.
  const SkColor zephyrus_card = zephyrus_background.value_or(
      SkColorSetRGB(0xf1, 0xf1, 0xf3));
  zephyrus_card_color_ = zephyrus_card;
  ...
  auto contents_host_builder =
      views::Builder<views::View>()
          ...
          // Zephyrus: the card (fill + shadow) is painted by the frame in
          // OnPaintBackground at GetZephyrusCardInset(), so it lands flush with
          // the search bar and its shadow isn't clipped by this layer. This
          // host stays transparent and just carries the rows.
          .SetPaintToLayer()
  ...
  // Zephyrus: no frame shadow. Chromium's BubbleBorder shadow is drawn around
  // the WHOLE frame rectangle, which spans the bar region on top plus the
  // gutters on the sides — so it reads as a translucent halo floating clear of
  // our inset white card (the "strange border surrounding the result view and
  // search bar"). The card is flat; the selected hero row carries its own
  // aligned shadow for depth.
  SetElevation(0);

  AddChildView(std::move(contents_host));

  // Zephyrus: the detached card never shows the bar "cutout" (the unified
  // bar+popup background) — the bar keeps its own pill above the gap.
  top_background_->SetVisible(false);
}
...
// static
int RoundedOmniboxResultsFrame::GetZephyrusCardInset() {
  // OmniboxPopupViewViews::GetTargetBounds() widens the popup so that, inside
  // GetContentsBounds(), there is exactly kZephyrusHeroOverhang of margin on
  // each side of the search bar. Insetting the card by that lands it flush with
  // the bar (Figma: bar and card are both 477 wide at the same x), leaving the
  // margin for the hero row to overhang into.
  return kZephyrusHeroOverhang;
}
...
void RoundedOmniboxResultsFrame::SetCutoutVisibility(bool visible) {
  // Zephyrus: the detached-card design never shows the cutout.
  visible = false;
...
void RoundedOmniboxResultsFrame::SetElevation(int elevation) {
  ...
  if (elevation > 0) {
    ...
    // Zephyrus: NO_SHADOW paints no halo, but the widget is still sized with
    // GetShadowInsets() margins (see OmniboxPopupViewViews::GetTargetBounds).
    // Keep those as layout insets so GetContentsBounds() still excludes the
    // margins — otherwise the card shifts up over the search bar and widens.
    border->set_insets(GetShadowInsets());
  }
...
gfx::Rect RoundedOmniboxResultsFrame::GetZephyrusCardBounds() const {
  if (!contents_host_) {
    return gfx::Rect();
  }
  const int inset = GetZephyrusCardInset();
  gfx::Rect card = contents_host_->bounds();
  card.Inset(gfx::Insets::TLBR(
      GetNonResultSectionHeight(true) + kZephyrusCardGap, inset, 0, inset));
  return card;
}

void RoundedOmniboxResultsFrame::OnPaintBackground(gfx::Canvas* canvas) {
  if (!contents_host_) {
    return;
  }
  // Zephyrus (Figma searchbar_resultdropdown): paint the DETACHED card here —
  // fill plus a soft drop shadow — inset by GetZephyrusCardInset() so it lands
  // flush with the search bar, separated from it by kZephyrusCardGap. Drawing
  // it in the frame (behind the transparent contents host) keeps the shadow
  // unclipped and lets the selected hero row overhang the card.
  const gfx::Rect card = GetZephyrusCardBounds();
  if (card.IsEmpty()) {
    return;
  }
  // No drop shadow. It was two downward-offset BLACK shadows, which on the
  // dark theme never read as depth — they just pooled into dark wedges either
  // side of the card's bottom corners. The card is lighter than the page, so
  // the fill alone is what separates it.
  cc::PaintFlags flags;
  flags.setAntiAlias(true);
  flags.setColor(zephyrus_card_color_);
  canvas->DrawRoundRect(gfx::RectF(card), kZephyrusCardRadius, flags);
}
...
gfx::Insets RoundedOmniboxResultsFrame::GetContentInsets() {
  // Zephyrus: rows always clear the bar region (even though the cutout is
