...
void OmniboxViewViews::SetEmphasis(bool emphasize, const gfx::Range& range) {
  ...
  // Zephyrus: when the title bar adopts the page color, the location bar
  // supplies an omnibox text color that contrasts with it.
  if (location_bar_view_) {
    if (std::optional<SkColor> zephyrus_color =
            location_bar_view_->GetZephyrusOmniboxTextColor()) {
      color = emphasize ? *zephyrus_color : SkColorSetA(*zephyrus_color, 0x99);
    }
  }
  if (range.IsValid()) {
