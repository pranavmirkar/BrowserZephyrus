...
gfx::ImageSkia GetFaviconForWebContents(content::WebContents* web_contents) {
  ...
  // Zephyrus: the well behind the favicon is an M3 surface, not a literal.
  // Upstream painted it pure BLACK on a dark theme and pure WHITE on a light
  // one, which on an M3 dialog reads as a hole punched through the card.
  // surfaceContainerHighest is the card-in-a-dialog role, one tone off the
  // bubble's own surfaceContainerHigh.
  const SkColor background_color =
      color_provider.GetColor(kColorZephyrusSurfaceContainerHighest);
