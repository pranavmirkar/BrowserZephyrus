...
void LocationBarView::Init() {
  ...
  omnibox_view_->Init();
  // Zephyrus: Safari-style — the steady-state domain reads centered in the
  // pill; editing switches to left alignment (see OnOmniboxFocused/Blurred).
  omnibox_view_->SetHorizontalAlignment(gfx::ALIGN_CENTER);
  // The placeholder ("Search … or type a URL") is drawn with its own flags and
  // does NOT follow the text alignment, so center it explicitly to match.
  omnibox_view_->set_placeholder_text_draw_flags(gfx::Canvas::TEXT_ALIGN_CENTER);
...
void LocationBarView::Layout(PassKey) {
  ...
  // Zephyrus: keep the search bar clean — strip every trailing icon (page
  // actions incl. PWA install, cookie controls, the bookmark star, content
  // settings, intent chip, AI-mode hint). Only the leading magnifier and the
  // text remain; the transient "clear text" button stays while editing.
  constexpr bool kZephyrusCleanSearchBar = true;
  if (kZephyrusCleanSearchBar) {
    // Actually hide the views — skipping the decoration alone leaves them
    // visible at stale bounds (they drifted left of the pill). Their
    // controllers may re-show them on state changes, but Layout runs after,
    // so hiding here every pass keeps the bar clean.
    auto hide = [](views::View* v) {
      if (v) {
        v->SetVisible(false);
      }
    };
    hide(page_action_icon_container_);
    hide(page_action_container_);
    hide(ai_mode_hint_label_);
    hide(intent_chip_);
  ...
  if (!kZephyrusCleanSearchBar) {
    ...
    for (ContentSettingImageView* view :
         base::Reversed(content_setting_views_)) {
      add_trailing_decoration(view, kContentSettingIntraItemPadding,
                              /*edge_padding=*/trailing_decorations_edge_padding);
    }
    if (intent_chip_) {
...
SkColor LocationBarView::GetIconLabelBubbleSurroundingForegroundColor() const {
  ...
  if (ShouldShowKeywordBubble()) {
    return GetColorProvider()->GetColor(kColorOmniboxKeywordSeparator);
  }
...
SkColor LocationBarView::GetIconLabelBubbleBackgroundColor() const {
  // Zephyrus: dynamic bar surface (page-adapted), matching the pill fill.
  return ZephyrusSurfaceColor();
}
...
void LocationBarView::SetZephyrusTitlebarColor(std::optional<SkColor> color) {
  if (zephyrus_titlebar_color_ == color) {
    return;
  }
  zephyrus_titlebar_color_ = color;
  RefreshBackground();
  // Re-apply text emphasis so the omnibox text picks up the adapted color.
  if (omnibox_view_) {
...
SkColor LocationBarView::ZephyrusSurfaceColor(bool hovered) const {
  // Zephyrus: elevate off the permanent theme base. The title bar pushes that
...
std::optional<SkColor> LocationBarView::GetZephyrusOmniboxTextColor() const {
...
void LocationBarView::AnimationEnded(const gfx::Animation* animation) {
  if (animation == &loading_animation_) {
    if (zephyrus_loading_) {
      loading_animation_.Start();  // Loop while still loading.
    }
    return;
  }
  DCHECK_EQ(animation, &hover_animation_);
...
void LocationBarView::SetZephyrusLoading(bool loading) {
  if (zephyrus_loading_ == loading) {
    return;
  }
...
void LocationBarView::OnOmniboxFocused() {
  ...
  // Zephyrus: editing wants left-aligned text (caret work reads left→right).
  omnibox_view_->SetHorizontalAlignment(gfx::ALIGN_LEFT);
  omnibox_view_->set_placeholder_text_draw_flags(gfx::Canvas::TEXT_ALIGN_LEFT);

  // Zephyrus: the compact pill shows only the domain in steady state. On focus,
  // reveal the FULL URL and select it (Safari behavior) so it can be read,
  // copied, or replaced. Chrome unelides on user-initiated focus, but the
  // compact-pill mouse path can leave it elided — force it here.
  if (omnibox_view_ && omnibox_controller_ &&
      omnibox_controller_->edit_model()) {
    if (omnibox_controller_->edit_model()->Unelide()) {
      omnibox_view_->SelectAll(true);
    }
  }

  // Zephyrus: grow the compact pill to its editing width (animated).
  if (browser_) {
    if (BrowserView* browser_view =
            BrowserView::GetBrowserViewForBrowser(browser_)) {
      if (ToolbarView* toolbar = browser_view->toolbar()) {
        toolbar->SetZephyrusOmniboxFocused(true);
      }
    }
  }
...
void LocationBarView::OnOmniboxBlurred() {
  ...
  // Zephyrus: back to the centered steady-state domain.
  omnibox_view_->SetHorizontalAlignment(gfx::ALIGN_CENTER);
  omnibox_view_->set_placeholder_text_draw_flags(gfx::Canvas::TEXT_ALIGN_CENTER);

  // Zephyrus: settle back to the compact steady-state pill (animated).
  if (browser_) {
    if (BrowserView* browser_view =
            BrowserView::GetBrowserViewForBrowser(browser_)) {
      if (ToolbarView* toolbar = browser_view->toolbar()) {
        toolbar->SetZephyrusOmniboxFocused(false);
      }
    }
  }
...
ui::ImageModel LocationBarView::GetLocationIcon(
  ...
  // Zephyrus (Figma Search Bar): the pixel magnifier leads the pill while
  // editing/empty AND on ordinary pages (secure or neutral). Only warning
  // states (mixed content, dangerous, "Not secure" chips) keep the security
  // indicator — those signals must not be masked by a decorative icon.
  if (location_icon_view_) {
