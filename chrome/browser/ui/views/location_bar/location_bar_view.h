...
class LocationBarView
 ...
 public:
  ...
  // Zephyrus: tells the location bar the active title bar color so the omnibox
  // background and text adapt to it. std::nullopt restores theme colors.
  void SetZephyrusTitlebarColor(std::optional<SkColor> color);

  // Zephyrus: the elevated-card surface (search bar + results dropdown fill),
  // derived from the page-adapted title bar color. |hovered| brightens it.
  SkColor ZephyrusSurfaceColor(bool hovered = false) const;

  // Zephyrus: omnibox text color that contrasts with the adapted title bar,
  // or std::nullopt to use the theme color.
  std::optional<SkColor> GetZephyrusOmniboxTextColor() const;

  // Zephyrus: maps an omnibox/results color id to a page-color-adapted value
  // (background, text, dimmed, url, icon, and their selected/hovered variants),
  // or std::nullopt when no page color is active so the theme color is used.
  std::optional<SkColor> GetZephyrusOmniboxColor(ui::ColorId id) const;

  // Zephyrus: shows/hides a minimal blue loading ring that animates around the
  // omnibox border while the active page is loading.
  void SetZephyrusLoading(bool loading);
 ...
 private:
  ...
  // Zephyrus: loading ring animation (continuous while a page loads). 30fps —
  // a slow-moving 2px dash doesn't need 60Hz and this halves paint work during
  // every page load.
  gfx::LinearAnimation loading_animation_{this, /*frame_rate=*/30};
  bool zephyrus_loading_ = false;
  ...
  // Zephyrus: active title bar color (page color), if the omnibox should adapt.
