...
void ReloadButton::SetVisibleMode(Mode mode) {
  metrics_recorder_->OnChangeVisibleMode(ToRecorderButtonMode(visible_mode_),
                                         ...
                                         base::TimeTicks::Now());
  // Zephyrus: never play the reload<->stop Lottie animation. It renders through
  // the animated image container and overrides the custom pixel-block reload
  // vector icon set via SetVectorIcons() below; disabling it lets the static
  // custom glyph show. (The morph animation is a cosmetic loss we accept.)
  const bool play_animation = false;
