...
BackForwardButton::BackForwardButton(Direction direction,
  ...
  if (direction == Direction::kBack) {
    // Zephyrus: dotted-chevron back icon (same glyph at all sizes/touch).
    SetVectorIcons(kZephyrusBackArrowIcon, kZephyrusBackArrowIcon);
    ...
    // Zephyrus: dotted-chevron forward icon (mirror of back).
    SetVectorIcons(kZephyrusForwardArrowIcon, kZephyrusForwardArrowIcon);
