...
// Zephyrus: override a (sub)menu host widget's color mode to match the active
// page's brightness, so the app menu participates in the dynamic theme even
// though the browser chrome is forced dark. No-op when the page color is
// unknown or the menu's widget isn't realized yet.
void ApplyZephyrusMenuColorMode(Browser* browser, views::MenuItemView* menu) {
  if (!menu) {
    return;
  }
  ...
  const std::optional<SkColor> page_color =
      browser_view->GetZephyrusPageColor();
  if (!page_color) {
    return;
  }
  views::SubmenuView* submenu = menu->GetSubmenu();
  views::Widget* widget = submenu ? submenu->GetWidget() : nullptr;
  if (!widget) {
    return;
  }
  widget->SetColorModeOverride(color_utils::IsDark(*page_color)
...
void AppMenu::RunMenu(views::MenuButtonController* host) {
  ...
  // Zephyrus: tint the (now-shown) root menu to the active page's brightness.
  ApplyZephyrusMenuColorMode(browser_, root_);
}
...
void AppMenu::WillShowMenu(MenuItemView* menu) {
  // Zephyrus: tint each submenu to the active page's brightness as it opens.
  ApplyZephyrusMenuColorMode(browser_, menu);
