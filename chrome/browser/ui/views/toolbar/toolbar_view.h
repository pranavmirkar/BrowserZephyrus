...
class ToolbarView : public views::AccessiblePaneView,
 ...
 public:
  ...
  // Zephyrus: tints the title bar (toolbar) with the active web page's color.
  // Pass std::nullopt to fall back to the default toolbar theme color.
  // Crossfades from the current color instead of snapping.
  void SetZephyrusTitlebarColor(std::optional<SkColor> color);

  // Zephyrus: applies |color| (and every color derived from it) immediately,
  // with no crossfade. The animation calls this once per frame.
  void ApplyZephyrusTitlebarColor(std::optional<SkColor> color);

  // Zephyrus: animates the omnibox pill between its compact steady width and
  // its wider editing width. Called by LocationBarView on omnibox focus/blur.
  void SetZephyrusOmniboxFocused(bool focused);

  // Zephyrus: gives the Zephyrus action buttons (new-tab/pin/shield/home) a
  // subtle circular background that contrasts with the current title bar color.
  // The back/forward/reload arrows stay quiet (hover only).
  void UpdateZephyrusNavButtonBackgrounds(SkColor titlebar_color);
 ...
 private:
  ...
  // Zephyrus: creates the min/maximize/close window controls at the right end
  // of the toolbar (which doubles as the title bar). Updates the maximize/
  // restore glyph to match the current window state.
  void AddZephyrusWindowControls();
  void UpdateZephyrusWindowControls();
  ...
  // False until SetZephyrusTitlebarColor has applied at least once, so the
  // initial call always initializes button highlights even when the incoming
  // color equals the default (nullopt).
  bool zephyrus_titlebar_color_applied_ = false;
  // Crossfades the title bar between page colors (defined in the .cc).
  // While it runs, zephyrus_titlebar_color_ holds the lerped intermediate
  // color and zephyrus_color_target_ holds where it is heading.
  std::unique_ptr<ZephyrusColorTransition> zephyrus_color_transition_;
  std::optional<SkColor> zephyrus_color_target_;
  // Animates the omnibox pill's focus expansion (defined in the .cc).
  std::unique_ptr<ZephyrusOmniboxFocusAnimation>
