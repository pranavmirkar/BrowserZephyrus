...
class ToolbarButtonHighlightPathGenerator
 ...
 public:
  ...
  SkPath GetHighlightPath(const views::View* view) override {
    for (const views::View* ancestor = view->parent(); ancestor;
         ancestor = ancestor->parent()) {
      if (const auto* toolbar = views::AsViewClass<ToolbarView>(ancestor)) {
        SkRRect shape;
        if (toolbar->GetZephyrusButtonShape(view, &shape)) {
          return SkPath::RRect(shape);
        }
        break;
      }
    }
...
void ToolbarButton::StateChanged(ButtonState old_state) {
  ...
  // Titlebar containers are painted by the toolbar behind the glyphs. Nested
  // pinned actions must invalidate that shared painter as well as themselves.
  for (views::View* ancestor = parent(); ancestor; ancestor = ancestor->parent()) {
    if (views::AsViewClass<ToolbarView>(ancestor)) {
      ancestor->SchedulePaint();
      break;
    }
  }
