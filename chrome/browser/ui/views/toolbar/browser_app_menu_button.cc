...
void BrowserAppMenuButton::UpdateIcon() {
  // Zephyrus: the app menu glyph is HORIZONTAL (...), not the vertical kebab.
  //
  // Not a rotation -- ui/views ships more_horiz.icon, so this is the same mark
  // drawn on its own axis rather than a transform that would land the dots on
  // fractional pixels and soften them.
  //
  // It also sits better in this title bar: everything around it (the caption
  // chevrons, the workspace dots, the tab counter) runs horizontally, and a
  // vertical stack of dots was the only element fighting that grain.
  const gfx::VectorIcon& icon = views::kMoreHorizIcon;
