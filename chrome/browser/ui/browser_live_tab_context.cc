...
std::map<std::string, std::string> BrowserLiveTabContext::GetExtraDataForTab(
  ...
  // ZEPHYRUS: remember which workspace the tab was in, so reopening it puts it
  // back in that workspace's StoragePartition.
  //
  // A tab's partition is fixed when its WebContents is created, so the restore
  // path needs this before the tab exists. Nothing else carries it: the saved
  // per-tab workspace list only exists during session restore, and a closed tab
  // reopened later has no entry there. Without this, Ctrl+Shift+T on a tab from
  // an isolated workspace brings it back signed in as the other account.
  content::WebContents* zephyrus_contents =
      tab_strip_model_->GetWebContentsAt(index);
  ZephyrusWorkspaceStore* zephyrus_store =
      zephyrus_contents ? ZephyrusWorkspaceStore::GetForProfile(
                              Profile::FromBrowserContext(
                                  zephyrus_contents->GetBrowserContext()))
                        : nullptr;
  if (zephyrus_store) {
    const int zephyrus_workspace =
        zephyrus_store->GetWorkspaceForContents(zephyrus_contents);
    if (zephyrus_workspace != 0) {
      extra_data[zephyrus::kWorkspaceExtraDataKey] =
          base::NumberToString(zephyrus_workspace);
    }
  }
