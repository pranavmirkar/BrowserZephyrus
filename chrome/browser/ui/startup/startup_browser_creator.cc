...
StartupProfileMode GetStartupProfileMode(
  ...
  // Zephyrus: never show Chrome's multi-profile picker on startup. Profiles are
  // switched in-window via the title-bar profile pill, so the picker is
  // redundant and reads as generic Chrome. Always open the last-used profile's
  // window directly instead (the picker is still reachable on demand).
  return StartupProfileMode::kBrowserWindow;
}
...
void StartupBrowserCreator::RegisterLocalStatePrefs(
  ...
  // Zephyrus: promotions (What's New, first-run feature tours, browser promos)
  // default OFF. Enterprise policy can still switch them on; this only changes
