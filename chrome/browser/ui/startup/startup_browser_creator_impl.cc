...
Browser* StartupBrowserCreatorImpl::OpenTabsInBrowser(
  ...
  // Zephyrus: an empty tab list is legitimate here — with no new-tab page, a
  // plain launch opens a window with no tabs at all, which shows the empty
  // backdrop (see BrowserView::UpdateZephyrusEmptyState). Upstream assumed a
  // startup window always has at least one tab.
  const bool zephyrus_open_empty = tabs.empty();
  ...
  // The `zephyrus_open_empty` case deliberately wants a tabless window, so the
  // blank-tab backstop below must not undo it.
  if (!zephyrus_open_empty &&
...
StartupBrowserCreatorImpl::DetermineStartupTabs(
  ...
  if (is_incognito_or_guest || is_post_crash_launch) {
    // Zephyrus: upstream shows the new tab page here (deliberately, to avoid
    // getting the user stuck in a crash loop). We have no new-tab page, so
    // launch with no tabs instead — the window opens on the empty backdrop,
    // which serves the same purpose of not re-running whatever crashed.
    //
    // This is what put a stray "New Tab" in the sidebar after any unclean exit:
    // a force-killed or crashed browser makes the NEXT launch a post-crash
    // launch, so the page reappeared even though every other producer had been
    // removed. It never reproduced on a freshly created profile.
...
Browser* StartupBrowserCreatorImpl::RestoreOrCreateBrowser(
  ...
  // Zephyrus: upstream substituted the NTP here whenever there was nothing to
  // open (a plain launch, or a session restore that didn't happen). We have no
  // new-tab page, so the window simply opens with no tabs and shows the empty
  // backdrop; the user starts with Ctrl+T.
  browser = OpenTabsInBrowser(
      browser, process_startup, tabs,
