...
void AddInfoBarsIfNecessary(BrowserWindowInterface* browser,
  ...
  // Zephyrus: intentionally ships without Google API keys — the "Google API
  // keys are missing" banner is noise, not information. Never show it.
  ...
  // Zephyrus calm-browser rule: security warnings above stay; everything
  // below is promotional nagging (startup-launch, default-browser prompt,
  // pin-to-taskbar, PDF promo, session-restore promo) and is never shown.
  static constexpr bool kZephyrusShowStartupPromos = false;
  if (!kZephyrusShowStartupPromos) {
    return;
  }
