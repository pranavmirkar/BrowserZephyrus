...
StartupTabs StartupTabProviderImpl::GetNewTabPageTabsForState(
    const SessionStartupPref& /*pref*/) {
  // Zephyrus: there is no new-tab page, so "open the New Tab page on startup"
  // means open nothing — the window comes up with no tabs and shows the empty
  // backdrop, and the user starts with Ctrl+T. Sessions still restore normally;
  // this only replaces the NTP that upstream injected for every other case.
  return StartupTabs();
}
