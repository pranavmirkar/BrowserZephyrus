...
void FirstRunService::OpenFirstRunInternal() {
  // Zephyrus: never show the first-run profile picker / onboarding. Proceed
  // straight to opening the browser for the current profile — profiles are
  // switched in-window via the title-bar pill, so the FRE picker is redundant
  // and reads as generic Chrome.
  if (resume_task_callback_) {
