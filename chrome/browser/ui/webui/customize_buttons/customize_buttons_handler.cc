...
void CustomizeButtonsHandler::SetCustomizeChromeSidePanelVisible(
  ...
  // Zephyrus: this button opens OUR panel.
  //
  // Chromium's side panel is a pane of the window -- it pins itself to the
  // frame and paints the browser's themed background. Zephyrus's chrome is
  // cards floating in the window, so the theming UI lives in a Zephyrus panel
  // beside the page, the same construction as the sidebar and the agent panel.
  // See ZephyrusCustomizePanel.
  //
  // Handled BEFORE GetSidePanelControllerForActiveTab() below, which CHECKs:
  // taking our path must not depend on a side panel controller existing for
  // this tab.
  //
  // Windows that get no panel (popups, app windows) fall through to upstream,
  // which is the same test the agent panel uses.
  // Reached through the embedding context, not a browser lookup: this
  // Chromium has no browser_finder, and the WebUI already registers itself
  // there. GetBrowserViewForBrowser() takes the BrowserWindowInterface
  // directly, so no cast is needed either.
  if (BrowserWindowInterface* browser_window =
          webui::GetBrowserWindowInterface(web_ui_->GetWebContents())) {
    if (BrowserView* browser_view =
            BrowserView::GetBrowserViewForBrowser(browser_window)) {
      if (zephyrus::ZephyrusCustomizePanel* panel =
              browser_view->zephyrus_customize_panel()) {
        if (visible) {
          panel->Open();
        } else {
          panel->Close();
        }
        NotifyCustomizeChromeSidePanelVisibilityChanged(visible);
        return;
      }
    }
  }
