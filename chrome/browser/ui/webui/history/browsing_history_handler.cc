...
void BrowsingHistoryHandler::OpenClearBrowsingDataDialog() {
  ...
  // Zephyrus: chrome://history is hosted outside a tab (the centralized
  // settings popup), so FindBrowserWithTab returns null. Open Clear Browsing
  // Data *in place* by navigating the host contents to the settings subpage,
  // rather than spawning a background tab in the parent window (which also
  // crashed upstream by dereferencing the null browser).
  if (web_contents_) {
