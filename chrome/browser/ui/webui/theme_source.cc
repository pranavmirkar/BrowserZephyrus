...
std::optional<std::string> ThemeSource::GenerateColorsCss(
  ...
  const std::vector<ColorSetDefinition> definitions = {
      ...
      // Zephyrus: the M3 roles, as --color-zephyrus-*. Stops short of the
      // legacy shim roles, which have no CSS names on purpose.
      {"zephyrus", kColorZephyrusPrimary, kColorZephyrusLegacyGround,
       base::BindRepeating(&ZephyrusColorIdCssName)},
#if BUILDFLAG(IS_CHROMEOS)
