...
void AddAboutStrings(content::WebUIDataSource* html_source, Profile* profile) {
  ...
  // Zephyrus product version, shown on the About page above the engine version.
  html_source->AddString("aboutZephyrusVersion",
                         u"Zephyrus " + std::u16string(zephyrus::kVersion));
  // Company website and an explicit credit to the Chromium project. Raw HTML
  // (rendered via $i18nRaw) so the links are clickable, matching how the
  // product-license line already embeds links.
  html_source->AddString(
      "aboutZephyrusCompany",
      u"Visit <a target=\"_blank\" href=\"https://thelazarus.in\">"
      u"thelazarus.in</a>");
  html_source->AddString(
      "aboutZephyrusChromiumCredit",
      u"Zephyrus is built on the <a target=\"_blank\" "
      u"href=\"https://www.chromium.org\">Chromium</a> open source project.");
  // Override the About-page title and help link, which resolve to "Chromium"
  // for this unofficial build. These AddString calls run after the localized
  // string batch, so they win.
  html_source->AddString("aboutPageTitle", u"About Zephyrus");
  html_source->AddString("aboutGetHelpUsingChrome", u"Get help with Zephyrus");
  html_source->AddString(
...
void AddZephyrusAdblockStrings(content::WebUIDataSource* html_source) {
  ...
  html_source->AddString(
      "zephyrusAdblockEnabledSubLabel",
      u"Blocks ads, trackers and pop-ups, and hides ad elements on pages");
  html_source->AddString("zephyrusAdblockPopupLabel",
                         u"Aggressive pop-up blocking");
  html_source->AddString(
      "zephyrusAdblockPopupSubLabel",
      u"Also block cross-site pop-ups and pop-unders that aren't in any filter "
      u"list");
  html_source->AddString("zephyrusAdblockStatsLabel", u"Statistics");
  html_source->AddString("zephyrusAdblockStatsValue",
                         u"$1 items blocked · $2 rules active");
  html_source->AddString("zephyrusAdblockAllowlistLabel", u"Allowed sites");
  html_source->AddString("zephyrusAdblockAllowlistSubLabel",
                         u"Blocking is turned off on these sites");
  html_source->AddString("zephyrusAdblockAllowlistPlaceholder", u"example.com");
  html_source->AddString("zephyrusAdblockAllowlistEmpty",
...
void AddLocalizedStrings(content::WebUIDataSource* html_source,
  ...
  AddZephyrusAdblockStrings(html_source);
