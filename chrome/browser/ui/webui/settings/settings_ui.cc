...
SettingsUI::SettingsUI(content::WebUI* web_ui)
  ...
  // Zephyrus: ContentAutofillClient is a tab helper and is null when settings
  // is hosted outside a browser tab (e.g. the Zephyrus settings popup's
  // WebView). Upstream unconditionally dereferenced it and crashed there.
  autofill::ContentAutofillClient* autofill_client =
      autofill::ContentAutofillClient::FromWebContents(
          web_ui->GetWebContents());
  html_source->AddBoolean("shouldShowPayOverTimeSettings",
                          autofill_client &&
  ...
  // Zephyrus: reuse the null-checked |autofill_client| from above — this
  // upstream deref also crashed when settings is hosted outside a tab.
  html_source->AddBoolean(
  ...
  // Keep emitting the per-feature booleans (other WebUI code reads them), but
  // Zephyrus force-hides the AI page entirely below.
  for (auto [name, visible] : optimization_guide_features) {
    html_source->AddBoolean(name, visible || show_ai_settings_for_testing);
  }

  // Zephyrus: no AI — always hide the entire AI settings page and its sections.
