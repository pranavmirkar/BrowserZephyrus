...
void MostVisitedHandler::OnURLsAvailable(
  ...
  for (auto& tile : sections.at(ntp_tiles::SectionType::PERSONALIZED)) {
    // Zephyrus: a tile drawn from the shared history appears only in the
    // workspace the site was used in. See zephyrus_most_visited_filter.h.
    if (tile.source == ntp_tiles::TileSource::TOP_SITES &&
        !zephyrus::KeepMostVisitedTile(web_contents_, tile.url)) {
      continue;
    }
