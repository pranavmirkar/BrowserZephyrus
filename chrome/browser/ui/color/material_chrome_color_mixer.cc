...
void AddMaterialChromeColorMixer(ui::ColorProvider* provider,
  ...
  // Toolbar colors.
  // Zephyrus (from Helium): in dark mode the toolbar is the FRAME colour, not
  // the base.
  //
  // This one line is the difference between a themed browser that reads as a
  // browser and one that reads as a single flat rectangle. kColorSysBase is
  // also what the page card and the panels resolve to, so pointing the toolbar
  // at it too erases every boundary between them the moment a saturated theme
  // is picked -- the exact failure of 2026-09-10. The frame is a distinct tone
  // in the same ramp, so the toolbar keeps its own plane at every seed colour.
  mixer[kColorToolbar] = {dark_mode ? ui::kColorFrameActive : ui::kColorSysBase};
  ...
  // Rebased per mode with the toolbar above: a divider tuned against
  // kColorSysBase is invisible against the frame.
  mixer[kColorToolbarContentAreaSeparator] = {
      dark_mode ? ui::kColorSysStateHoverOnSubtle : ui::kColorSysSurfaceVariant};
  ...
  // Blended against what is actually BEHIND the separator. In dark mode the
  // toolbar is now the frame, so blending against kColorToolbar would compute
  // the separator against the wrong plane.
  const auto toolbar_separator_bg = dark_mode
                                        ? ui::ColorTransform(ui::kColorSysBase)
                                        : ui::ColorTransform(kColorToolbar);
  mixer[kColorToolbarSeparatorDefault] =
      ui::AlphaBlend(kColorToolbarButtonIcon, toolbar_separator_bg, 0x3A);
