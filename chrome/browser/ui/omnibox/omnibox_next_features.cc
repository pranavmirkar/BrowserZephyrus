...
//
// ZEPHYRUS: forced back to DISABLED. Upstream flipped this to ENABLED between
// 151.0.7913 and 151.0.7922, which silently swapped the whole dropdown
// implementation: LocationBarView then builds an OmniboxPopupViewWebUI instead
// of falling through to the legacy OmniboxPopupViewViews.
//
// Every piece of the Zephyrus omnibox design targets the VIEWS popup —
// RoundedOmniboxResultsFrame's detached card and hero overhang,
// OmniboxResultView's hero pill and width-jump animation, the engine pill. None
// of it renders in the WebUI popup, so the dropdown came apart visually while
// every one of those files still compiled and every test still passed: the code
// was not wrong, it simply stopped being called.
//
// Re-enabling this means porting the Figma dropdown to the WebUI popup first.
// Until then Zephyrus stays on the Views implementation deliberately, and this
// override is the thing keeping the design alive.
...
//
// ZEPHYRUS: also forced back to DISABLED, same upstream flip. It takes over the
// entire popup — including covering the location bar — which is not a surface
// this browser offers.
