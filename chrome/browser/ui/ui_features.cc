...
bool IsWebUIReloadButtonEnabled() {
  // Zephyrus: always use the NATIVE reload button. The WebUI toolbar reload
  // button renders its own icon and ignores the native ReloadButton's custom
  // pixel-block reload glyph, so force the native path.
  return false;
}
