...
std::vector<content::WebContents*> TabMatcherDesktop::GetOpenWebContents(
  ...
  // Zephyrus: only tabs in the workspace the omnibox is serving. "Switch to
  // tab" offered every workspace's tabs, which both shows one workspace's open
  // pages in another and does nothing visible when chosen -- the window snaps
  // back to its own workspace the moment a foreign tab activates.
  ZephyrusWorkspaceStore* const store =
      ZephyrusWorkspaceStore::GetForProfile(profile_);
  const int workspace = store ? store->CurrentWorkspaceForOmnibox() : 0;
  ...
  for (BrowserWindowInterface* bwi : GetAllBrowserWindowInterfaces()) {
    ...
    for (int i = 0; i < tab_strip_model->count(); ++i) {
      ...
      if (workspace != 0) {
        const int tab_workspace = store->GetWorkspaceForContents(web_contents);
        if (tab_workspace != 0 && tab_workspace != workspace) {
          continue;
        }
      }
