...
// Permitted for this file specifically; see chrome/browser/autocomplete/DEPS.
...
history::HistoryService* ChromeAutocompleteProviderClient::GetHistoryService() {
  // Zephyrus Private Workspace: HistoryServiceFactory is
  // kRedirectedToOriginal, so an off-the-record profile shares the REGULAR
  // profile's history. That is stock incognito behavior — Chrome deliberately
  // autocompletes incognito typing from your normal history — but it defeats
  // the point of a private workspace and puts your normal browsing on screen
  // for anyone looking. Cut it off at the source so no history-backed provider
  // can reach it.
...
ChromeAutocompleteProviderClient::GetTopSites() {
  // Private Workspace: same redirect-to-original problem as GetHistoryService().
  if (profile_ && profile_->IsOffTheRecord()) {
    return nullptr;
  }
...
bool ChromeAutocompleteProviderClient::SearchSuggestEnabled() const {
  // Private Workspace sends nothing to the search provider while you type.
  // Suggest-as-you-type streams every keystroke to Google (or whichever engine
  // is set) before you ever press Enter, which is exactly the background
  // connection this mode exists to cut. Spec item 14.
  if (profile_ && profile_->IsOffTheRecord()) {
    return false;
  }
#if BUILDFLAG(IS_CHROMEOS)
...
bool ChromeAutocompleteProviderClient::IsUrlOutsideCurrentWorkspace(
