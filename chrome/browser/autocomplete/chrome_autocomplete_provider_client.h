...
class ChromeAutocompleteProviderClient : public AutocompleteProviderClient {
 ...
 private:
  // NOTE: the OTR omnibox guards gate on `profile_` (the window's own profile),
  // which is deterministic and correct for the shipping window-swap Private
  // Workspace (the private window is a real OTR Browser). PW-6 (in-window mixed
  // profiles) will need an active-tab-profile resolver here instead — see the
  // zephyrus-mixed-profile-window memory.
