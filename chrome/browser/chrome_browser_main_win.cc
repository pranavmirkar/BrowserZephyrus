...
// Zephyrus: brand font for all native browser UI (title bar, menus, bubbles,
// settings-popup rail, ...) — Inter. Locale-specific adjustments run first so
// scale fixes still apply; the family is then overridden unconditionally.
// Requires the font to be installed on the system; DirectWrite falls back to
// the default UI font when it is missing.
void ZephyrusAdjustUiFont(gfx::win::FontAdjustment& font_adjustment) {
  l10n_util::AdjustUiFont(font_adjustment);
  font_adjustment.font_family_override = L"Inter";
}
...
void ChromeBrowserMainPartsWin::ToolkitInitialized() {
  ...
  gfx::win::SetAdjustFontCallback(&ZephyrusAdjustUiFont);
