...
void ChromeContentBrowserClient::
  ...
  // Zephyrus §6.5: a service worker fetches its own seed. Registered here,
  // ABOVE the extensions block below, because that block returns early for
  // every non-extension scheme — which is every service worker this is for.
  //
  // Registered unconditionally, feature on or off. A service worker asking for
  // an interface that has no binder is a bad Mojo message and costs the whole
  // renderer process; answering "empty seed" is the only safe way to decline.
  map->Add<zephyrus_privacy::mojom::FingerprintSeedHost>(
      &zephyrus_privacy::ZephyrusFingerprintSeedServiceWorkerHost::Create);
