...
// Zephyrus: kept at Chrome's default of 2 (was briefly raised to 4 for RAM, but
// cramming 4 same-site heavy tabs into one renderer made animation-heavy sites
// jank — Chrome shares only 2).
...
// Zephyrus: disabled — the warm spare is an empty renderer (~50-150MB) idling
// at all times purely to shave startup latency off the next navigation. For a
// lowest-RAM profile that trade is wrong; navigations spawn their renderer on
// demand instead.
