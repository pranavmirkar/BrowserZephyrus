...
LRESULT LegacyRenderWidgetHostHWND::OnNCHitTest(UINT message,
  ...
  // For window-resize hit codes, pass through to the parent HWND so Windows
  // initiates a resize of the top-level browser window rather than this child.
  if (hit_test == HTLEFT || hit_test == HTRIGHT || hit_test == HTTOP ||
      hit_test == HTBOTTOM || hit_test == HTTOPLEFT ||
      hit_test == HTTOPRIGHT || hit_test == HTBOTTOMLEFT ||
...
LRESULT LegacyRenderWidgetHostHWND::OnSetCursor(UINT message,
                                                WPARAM w_param,
                                                LPARAM l_param) {
  // When the cursor is in the resize border of the parent window, set the
  // appropriate resize cursor and return TRUE to stop further processing.
  // This prevents the Aura cursor manager from overriding the native resize
  // cursor with the web-content cursor on every WM_SETCURSOR message.
  ...
  if (parent && !::IsZoomed(parent)) {
    ...
    LPCWSTR cursor_res = nullptr;
    if ((on_top && on_left) || (on_bottom && on_right))
      cursor_res = IDC_SIZENWSE;
    else if ((on_top && on_right) || (on_bottom && on_left))
      cursor_res = IDC_SIZENESW;
    else if (on_left || on_right)
      cursor_res = IDC_SIZEWE;
    else if (on_top || on_bottom)
      cursor_res = IDC_SIZENS;
    if (cursor_res) {
      ::SetCursor(::LoadCursor(nullptr, cursor_res));
      return TRUE;
    }
  }
  return 0;
