...
bool RenderWidgetHostViewAura::ShouldSkipCursorUpdate() const {
  ...
  // Skip cursor update when the cursor is in the resize border of the
  // top-level window so the native resize cursor from WM_SETCURSOR is
  // preserved and not overridden by the web-content cursor.
  // Use DIP-space coordinates throughout — cursor_screen_point is already
  // in DIP (from Screen::GetCursorScreenPoint), and root_window bounds
  // are also in DIP, so no pixel conversion is needed.
  {
    aura::WindowTreeHost* host = root_window->GetHost();
    if (host && !::IsZoomed(host->GetAcceleratedWidget())) {
      gfx::Rect win_dip = root_window->GetBoundsInScreen();
      constexpr int kResizeBorder = 6;  // DIP
      const int x = cursor_screen_point.x();
      const int y = cursor_screen_point.y();
      if (x < win_dip.x() + kResizeBorder ||
          x >= win_dip.right() - kResizeBorder ||
          y < win_dip.y() + kResizeBorder ||
          y >= win_dip.bottom() - kResizeBorder) {
        return true;
      }
    }
  }
...
void RenderWidgetHostViewAura::OnMouseEvent(ui::MouseEvent* event) {
#if BUILDFLAG(IS_WIN)
  // Zephyrus frameless resize: the web contents cover the entire top-level
  // window, including the outer edge pixels where the OS would normally
  // provide resize handles. Intercept events in that border here -- this is
  // the one place that reliably receives the mouse events -- and drive a
  // native resize via WM_SYSCOMMAND/SC_SIZE so dragging any edge resizes the
  // browser window.
  if (event->type() == ui::EventType::kMouseMoved ||
    ...
    if (top_hwnd && !::IsZoomed(top_hwnd) && !::IsIconic(top_hwnd) &&
        (::GetWindowLong(top_hwnd, GWL_STYLE) & WS_THICKFRAME)) {
      POINT screen_pt;
      ::GetCursorPos(&screen_pt);
      POINT client_pt = screen_pt;
      ::ScreenToClient(top_hwnd, &client_pt);
      RECT rc;
      ::GetClientRect(top_hwnd, &rc);
      const int dpi = ::GetDpiForWindow(top_hwnd);
      const int kBorder = ::MulDiv(8, dpi, 96);
      const bool on_left = client_pt.x < kBorder;
      const bool on_right = client_pt.x >= rc.right - kBorder;
      const bool on_top = client_pt.y < kBorder;
      const bool on_bottom = client_pt.y >= rc.bottom - kBorder;
      int direction = 0;  // A WMSZ_* value.
      ...
      if (direction != 0) {
        if (event->type() == ui::EventType::kMousePressed &&
          ...
          // Release Aura's capture so the OS modal resize loop can run, then
          // ask Windows to begin resizing the top-level window.
          ::ReleaseCapture();
          ::SendMessage(top_hwnd, WM_SYSCOMMAND, SC_SIZE + direction,
                        MAKELPARAM(screen_pt.x, screen_pt.y));
        } else {
          // Mouse moved over the border: show the resize cursor.
