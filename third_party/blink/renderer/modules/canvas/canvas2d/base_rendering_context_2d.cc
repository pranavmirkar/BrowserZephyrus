...
ImageData* BaseRenderingContext2D::getImageDataInternal(
  ...
  // Zephyrus §6.5/§9.2.1: report the touch regardless of whether we perturb.
  // Detection and randomization are independent — the attempt heuristic must
  // still see this surface when randomization is off, or turning the flag off
  // would also blind the report.
  ZephyrusReportFingerprintSurface(GetTopExecutionContext(),
                                   zephyrus_privacy::mojom::blink::FingerprintSurface::kCanvasRead);

  // Zephyrus §6.5: perturb the bytes the page is about to see.
  //
  // Applied HERE, after the real readback, because this is the single point
  // every getImageData() path converges on — hooking earlier would miss the
  // out-of-bounds and zero-initialised branches above, and hooking in the
  // caller would have to be repeated for each overload.
  if (std::optional<std::array<uint8_t, 32>> seed =
          ZephyrusSeedForSurface(GetTopExecutionContext(), kZephyrusFpCanvas)) {
    SkPixmap pixmap = image_data->GetSkPixmap();
    if (auto* addr = static_cast<uint8_t*>(pixmap.writable_addr())) {
      ApplyZephyrusCanvasNoise(
          base::span(*seed),
          // UNSAFE_BUFFERS: the size comes from Skia's own description of the
          // buffer it just allocated for this ImageData.
          UNSAFE_BUFFERS(base::span(addr, pixmap.computeByteSize())),
          // The requested rect in canvas coordinates, so this read agrees
          // with any other read of the same pixels.
          sw, sx, sy);
    }
  }
