...
class MODULES_EXPORT MediaDevices final
 ...
 private:
  ...
  // Zephyrus §9.2.1: reports that this document read the device list, which
  // leaks device labels and stable IDs with no permission prompt. Observation
  // only — nothing on this pipe can fail, delay or alter enumerateDevices().
  // Bound lazily on the first report, so a page that never calls
  // enumerateDevices() never creates it.
  void ReportZephyrusDeviceEnumeration(LocalFrame* frame);
  HeapMojoRemote<zephyrus_privacy::mojom::blink::PrivacyReporter>
      zephyrus_privacy_reporter_;
