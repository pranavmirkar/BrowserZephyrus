...
MediaDevices::MediaDevices(Navigator& navigator)
    : ActiveScriptWrappable<MediaDevices>({}),
      ...
      zephyrus_privacy_reporter_(navigator.GetExecutionContext()),
...
ScriptPromise<IDLSequence<MediaDeviceInfo>> MediaDevices::enumerateDevices(
  ...
  // Zephyrus §9.2.1. Deliberately before the dispatcher call and deliberately
  // not on the result: the page ASKED for the device list, and that is the
  // fact being recorded. Whether the enumeration then succeeds, times out or
  // returns an empty list is not something the user's privacy record should
  // depend on.
  ReportZephyrusDeviceEnumeration(frame);
...
void MediaDevices::ReportZephyrusDeviceEnumeration(LocalFrame* frame) {
  DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
  ExecutionContext* const execution_context = GetExecutionContext();
  if (!frame || !execution_context) {
    return;
  }
  if (!zephyrus_privacy_reporter_.is_bound()) {
    frame->GetBrowserInterfaceBroker().GetInterface(
        zephyrus_privacy_reporter_.BindNewPipeAndPassReceiver(
            execution_context->GetTaskRunner(TaskType::kMiscPlatformAPI)));
  }
  // Fire-and-forget. If the browser side is absent — the feature is off, or
  // this is an off-the-record profile where the service is deliberately never
  // created — the message is simply dropped, which is the intended behaviour
  // rather than an error to handle.
  zephyrus_privacy_reporter_->ReportFingerprintSurface(
      zephyrus_privacy::mojom::blink::FingerprintSurface::
...
void MediaDevices::Trace(Visitor* visitor) const {
  ...
  visitor->Trace(zephyrus_privacy_reporter_);
