...
class CORE_EXPORT NavigatorBase : public ScriptWrappable,
 ...
 public:
  ...
  // Zephyrus §6.5. Overridden here rather than in the mixin because this is the
  // first class in the hierarchy with an ExecutionContext, and the seed is per
  // (origin, session). Workers reach this too, and correctly get no
  // perturbation: they have no frame, so no seed.
