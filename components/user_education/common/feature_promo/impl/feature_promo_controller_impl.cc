...
FeaturePromoResult FeaturePromoControllerImpl::CanShowPromo(
  ...
  // Zephyrus: in-product promos are switched off wholesale. Every IPH bubble
  // ("Split view makes it easier to multitask", feature tours, "try this")
  // funnels through this method and MaybeShowPromo below, so declining here
  // silences the entire family rather than disabling features one by one.
  //
  // The original body is removed rather than left after an early return:
  // Chromium builds with -Wunreachable-code -Werror, so dead code does not
  // compile. Functional UI (permission prompts, download and security
  // warnings, page errors) is a different system and is unaffected.
...
void FeaturePromoControllerImpl::MaybeShowPromo(
    FeaturePromoParams params,
    UserEducationContextPtr context) {
  // Zephyrus: see CanShowPromo — no in-product promos. The result is reported
  // rather than dropped so callers' callbacks complete instead of hanging.
  PostShowPromoResult(*params.feature,
                      std::move(params.show_promo_result_callback),
