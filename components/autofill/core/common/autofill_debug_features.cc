...
// ZEPHYRUS: DISABLED. Upstream defaults this ON, and it is not gated on
// sign-in, consent, or a valid API key.
//
// Observed on the wire in a fresh profile (2026-08-24 netlog audit): four POSTs
// to https://content-autofill.googleapis.com/v1/pages/<base64> after loading
// bbc.com, github.com, wikipedia.org and youtube.com, each carrying that site's
// form signatures plus an x-client-data finch header.
//
// use_official_google_api_keys=false does NOT stop this. The empty key makes
// Google REJECT the reply — it does not prevent the request, and by then the
// connection has already told Google which site the user is on. The rejection
// is after the leak, not instead of it.
//
// No field VALUES are sent, only form structure. But "which sites you visit
// have forms" is browsing history by another name, and a browser whose whole
// premise is that trackers do not get to learn that cannot ship it on by
// default.
//
// Cost of turning it off, stated plainly: autofill loses Google's server-side
// field-type predictions and falls back to local heuristics. Saving and filling
// addresses, cards and passwords are unaffected — they were always local.
// Guarded by PrivacyNoUploadTest.AutofillCrowdsourcingIsDisabled.
