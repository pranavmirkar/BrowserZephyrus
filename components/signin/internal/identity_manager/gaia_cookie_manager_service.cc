...
void GaiaCookieManagerService::TriggerListAccounts() {
  // Zephyrus: never ask accounts.google.com which Google accounts are in the
  // cookie jar — the browser has no Google sign-in surface, and this request
  // fired on every startup regardless of the signin.allowed pref. The cached
  // (empty) cookie-jar info is returned to callers instead.
  static constexpr bool kZephyrusAllowListAccounts = false;
  if (!kZephyrusAllowListAccounts) {
    return;
  }
