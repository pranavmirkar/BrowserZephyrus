...
class AutocompleteProviderClient : public OmniboxAction::Client {
 public:
  ...
  // Zephyrus: true when `url` was not opened in the workspace currently on
  // screen, so history matches for it can be ranked below the ones that were.
  // Embedders without workspaces (and any state where scoping can't be applied
  // safely, such as a workspace with no recorded visits yet) return false,
