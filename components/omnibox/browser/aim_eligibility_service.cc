...
void AimEligibilityService::StartServerEligibilityRequest(
    RequestSource request_source,
    const std::string& locale) {
  // Zephyrus: AI Mode is not part of this browser (its entry points are
  // removed), and this request phones Google (/async/folae) on every startup
  // and periodically after — by its own traffic annotation it is not even
  // gated by policy. This is the single funnel every scheduling path
  // converges on (FetchEligibility, ScheduleServerEligibilityRequest, OAuth
  // continuations); never fetch.
  static constexpr bool kZephyrusAllowAimEligibilityFetch = false;
  if (!kZephyrusAllowAimEligibilityFetch) {
    return;
  }
