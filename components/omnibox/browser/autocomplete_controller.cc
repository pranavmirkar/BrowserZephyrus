...
void AutocompleteController::UpdateResult(UpdateType update_type,
  ...
  // Zephyrus: rank history for URLs opened in OTHER workspaces below the ones
  // opened here, so browsing in one workspace doesn't crowd another's
  // suggestions. Deliberately a demotion, not a cull: everything stays
  // reachable, just lower. Applied before SortAndCull so ranking and
  // default-match selection run on the adjusted scores rather than being
  // second-guessed afterwards.
  //
