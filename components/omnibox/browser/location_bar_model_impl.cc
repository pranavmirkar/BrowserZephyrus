...
std::u16string LocationBarModelImpl::GetURLForDisplay() const {
  ...
  // Zephyrus: Safari-style steady state — the pill shows only the domain
  // ("youtube.com"). The full URL still appears the moment the omnibox is
  // focused for editing (that path uses GetFormattedFullURL()).
  format_types |= url_formatter::kFormatUrlTrimAfterHost;
