...
// Zephyrus: disabled — the ML scoring model rides on the optimization-guide
// model (which we've turned off to strip AI); leaving it on makes the omnibox
// wait forever for a model that never loads (suggestions stay stuck "loading").
// Falls back to Chrome's classic hand-tuned relevance scoring.
BASE_FEATURE(kMlUrlScoring, DISABLED);
...
// Zephyrus: disabled (see kMlUrlScoring) — no omnibox ML scoring model.
BASE_FEATURE(kUrlScoringModel, DISABLED);
