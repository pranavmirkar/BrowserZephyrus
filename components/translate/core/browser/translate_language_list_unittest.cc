...
TEST_F(TranslateLanguageListTest, GetSupportedLanguagesFetch) {
  // Zephyrus: the roster fetch is off by default (see kTranslateLanguageListFetch
  // in translate_features.cc -- it is an unprompted contact with Google). The
  // machinery is gated, not deleted, so this test opts back in to keep covering
  // it; without this the interceptor below never fires and loop.Run() hangs
  // rather than failing. The shipped default is asserted separately, in
  // chrome/browser/zephyrus/privacy/no_unprompted_google_contact_unittest.cc.
  base::test::ScopedFeatureList feature_list;
  feature_list.InitAndEnableFeature(kTranslateLanguageListFetch);
