...
// Zephyrus: the second unprompted Google contact in the translate stack.
//
// With either of these on, the constructor below builds a RankerModelLoaderImpl
// and immediately calls NotifyOfRankerActivity(), which downloads a model from
// www.gstatic.com/chrome/intelligence/assist/ranker/models/ on any profile
// whose cache lacks one. No user action is involved.
//
// Disabling BOTH is the upstream-designed off switch, not a hack: the
// constructor then skips the loader entirely, GetModelURL() returns an empty
// GURL, and ShouldOfferTranslation() returns its documented kDefaultResponse of
// true. The cost is that the translate prompt is no longer ML-suppressed, so it
// is offered somewhat more often -- which is the behaviour Chrome had before
// the ranker existed, and a visible prompt is a far better failure mode than a
// silent background download of a 2017 model.
BASE_FEATURE(kTranslateRankerQuery, base::FEATURE_DISABLED_BY_DEFAULT);
BASE_FEATURE(kTranslateRankerEnforcement, base::FEATURE_DISABLED_BY_DEFAULT);
