...
void TranslateLanguageList::RequestLanguageList() {
  // Zephyrus: the single choke point for the roster fetch. Every path that can
  // reach the network -- GetSupportedLanguages(), SetResourceRequestsAllowed()
  // replaying a deferred request, and TranslatePrefs on a language change --
  // funnels through here, so gating here rather than at the three call sites
  // means a future caller cannot reintroduce the request by accident.
  //
  // request_pending_ is cleared rather than left set: SetResourceRequestsAllowed
  // calls this and then DCHECKs that nothing is still pending, so an early
  // return that skipped the clear would turn a disabled fetch into a debug
  // crash the moment resource requests are allowed.
  if (!base::FeatureList::IsEnabled(kTranslateLanguageListFetch)) {
    request_pending_ = false;
    return;
  }
