...
// Zephyrus: upstream this test was ModelLoaderQueryEnabledByDefault and
// asserted the opposite, because upstream ships kTranslateRankerQuery on. We
// ship it off: constructing the loader downloads a model from gstatic.com on
// any profile without a cached copy, with no user action (see the flag comment
// in translate_ranker_impl.cc).
//
// Deliberately left as a DEFAULT assertion -- only kTranslateRankerEnforcement
// is set explicitly -- so it keeps testing the shipped configuration rather
// than one the test invents. That makes it the tripwire for a rebase that
// silently restores the upstream default: it starts failing the moment the flag
// flips back, in the same file as the flag, which is where whoever resolved
// that conflict is already looking.
TEST_F(TranslateRankerImplTest, ModelLoaderQueryDisabledByDefault) {
  InitFeatures({}, {kTranslateRankerEnforcement});
  auto ranker = GetRankerForTest(0.01f);
  EXPECT_FALSE(ranker->CheckModelLoaderForTesting())
      << "a model loader was constructed with the ranker at its shipped "
         "default, which means the gstatic.com model download is live again";
}
