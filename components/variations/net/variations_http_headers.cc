...
// Zephyrus: no X-Client-Data header, ever.
//
// The header carries the set of field-trial groups this browser is in, on every
// request to a Google property. The combination is close to unique, so it acts
// as a quiet identifier -- and we run no experiments, so it would carry nothing
// useful even if we wanted it.
//
// This is the single producer: all three attach sites obtain the value here,
// and every one already treats std::nullopt as "no header", because upstream
// returns nullopt whenever the variation ID list is empty. So this removes the
// header rather than sending an empty one -- an empty X-Client-Data would
