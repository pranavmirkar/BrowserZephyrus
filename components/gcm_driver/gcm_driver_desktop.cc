...
GCMClient::Result GCMDriverDesktop::EnsureStarted(
  ...
  // Zephyrus: lazy check-in — while no site has been granted notification
  // permission (see the embedder-installed callback), never start the GCM
  // client, which suppresses the device check-in to
  // android.clients.google.com on fresh profiles. Evaluated on every start
  // attempt, so the first permission grant enables GCM for the subscribe()
  // that follows it.
  if (!ZephyrusGcmStartAllowed())
