...
class GCMDriver {
 public:
  ...
  // Zephyrus: optional gate consulted before the GCM client is (re)started.
  // While the callback returns false, starting GCM — and therefore the device
  // "check-in" to android.clients.google.com — is suppressed. The embedder
  // wires this to "has the user granted any site notification permission?",
  // so the first real grant lazily enables GCM. Unset means always allowed.
  void SetZephyrusStartAllowedCallback(base::RepeatingCallback<bool()> allowed) {
    zephyrus_start_allowed_callback_ = std::move(allowed);
  }

  // True when GCM may start (no callback installed, or it returns true).
  bool ZephyrusGcmStartAllowed() const {
    return !zephyrus_start_allowed_callback_ ||
           zephyrus_start_allowed_callback_.Run();
  }
 ...
 private:
  // Zephyrus: see SetZephyrusStartAllowedCallback().
  base::RepeatingCallback<bool()> zephyrus_start_allowed_callback_;
