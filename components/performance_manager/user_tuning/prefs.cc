...
void RegisterLocalStatePrefs(PrefRegistrySimple* registry) {
  ...
  // Zephyrus: Memory Saver ON by default to save RAM, but with Chrome's default
  // MEDIUM aggressiveness and a longer discard delay so heavy/animation-heavy
  // background tabs aren't discarded eagerly (which made them reload — showing a
  // blank page + janky re-init — when you switched back). Tunable in
  // Settings > Performance.
  registry->RegisterIntegerPref(kMemorySaverModeTimeBeforeDiscardInMinutes, 120);
